%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% adjustRGB1.m: Takes in the set of initial x, y, Y values produced as a 
% result of using the starting R, G, and B values. It adjusts the values of
% R, G, and B using <temporary estimated mathematical relationship between
% RGB & CIE colour spaces> to approach the acceptable ranges for the
% corresponding target x, y, and Y values. The function returns the 
% corresponding final R, G, B values.
%
% # Currently, program only saves the final RGB and xyY values, and does not
% store intermediate working values.
%
% Bits++ Compatible version
%
% Author: Jiun-Yiing Hu, 07/13
% Last modified: Josh, sometime in early 2014
% Dependencies: runPR655.m
% Client: autocalibrate.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


function [R_f, G_f, B_f, x_f, y_f, Y_f, triesMat, spec] = adjustRGB1new(x, y, Y, R, G, B, x_t, y_t, Y_t, satName, colorName, count, Clut, Yname)
...
while fValsObtained == 0
	...
	if dx == 0 & dy == 0 & dY == 0
	[fValsObtained finalGuesses triesMat n, spec] = checkFinalValues(x_t, y_t, Y_t, R, G, B, Clut, n, triesMat);
	end
...
figName = sprintf('%s_sat%s_%sL_Fig',colorName,satName(3:end),Yname);
cd Guesses
...
function [fValsObtained finalGuesses triesMat n spec] = checkFinalValues(x_t, y_t, Y_t, R, G, B, Clut, n, triesMat)
finalGuesses = [];
for i = 1:6
[x, y, Y, spec] = runPR655J1(R, G, B, Clut);
