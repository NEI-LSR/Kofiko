...
fields = fieldnames(g_strctConfig.m_strctEnabledPlots);
g_strctStatistics.numEnabledPlots = 0;

for iFields = 1:size(fields,1)
    g_strctStatistics.numEnabledPlots = g_strctStatistics.numEnabledPlots + g_strctConfig.m_strctEnabledPlots.(fields{iFields});
end
...
for iUnit = 1:numUnitsInUpdate
    ...
    plotRowInUse = 1;
    if g_strctConfig.m_strctEnabledPlots.m_bPSTHEnabled && ~isempty(g_strctStatistics.trackedDKLCoordinates)
...
imagesc(colorBar);
    ...
    if g_strctConfig.m_strctEnabledPlots.m_bLUVPSTHEnabled && ~isempty(g_strctStatistics.trackedLUVCoordinates)
    ...
    %}
    if g_strctConfig.m_strctEnabledPlots.m_bPositionHeatmapEnabled
        PositionPSTHHolder = zeros(prod(g_strctStatistics.m_aiSpikePositionBinGrid),1);
        for indices = 1:prod(g_strctStatistics.m_aiSpikePositionBinGrid)
    ...
    if max(max(max(any(PSTHOrientationHolder)))) && g_strctConfig.m_strctEnabledPlots.m_bOrientationPolarEnabled
    ...
    if g_strctConfig.m_strctEnabledPlots.m_bVariableWidthPlotEnabled && (g_strctStatistics.numAvailableSubplotRows >= 1)
        subplot(g_strctStatistics.numEnabledPlots,g_strctStatistics.numCurrentlyTrackedUnits,unitsInUpdate(iUnit) + (g_strctStatistics.numCurrentlyTrackedUnits * ...
            (g_strctStatistics.numEnabledPlots-g_strctStatistics.numAvailableSubplotRows)));
    ...
    if g_strctConfig.m_strctEnabledPlots.m_bVariableLengthPlotEnabled && (g_strctStatistics.numAvailableSubplotRows >= 1)
        subplot(g_strctStatistics.numEnabledPlots,g_strctStatistics.numCurrentlyTrackedUnits,unitsInUpdate(iUnit) + (g_strctStatistics.numCurrentlyTrackedUnits * ...
            (g_strctStatistics.numEnabledPlots-g_strctStatistics.numAvailableSubplotRows)));
    ...
    if g_strctConfig.m_strctEnabledPlots.m_bVariableSpeedPlotEnabled && (g_strctStatistics.numAvailableSubplotRows >= 1)
        subplot(g_strctStatistics.numEnabledPlots,g_strctStatistics.numCurrentlyTrackedUnits,unitsInUpdate(iUnit) + (g_strctStatistics.numCurrentlyTrackedUnits * ...
            (g_strctStatistics.numEnabledPlots-g_strctStatistics.numAvailableSubplotRows)));
   ...
   % dbstop if warning
    ...
    if g_strctConfig.m_strctEnabledPlots.m_bImagePlotEnabled && isfield(g_strctStatistics,'m_strctImageAnalysis') && ~isempty(g_strctStatistics.m_strctImageAnalysis)
...
if ~isfield(trialToProcess,'m_aiStimxyY') && isfield(trialToProcess,'m_iXCoordinate') || isfield(trialToProcess,'m_aiStimXYZ') && ~any(isnan(trialToProcess.m_aiStimXYZ))
                ...
                elseif isfield(trialToProcess,'m_aiStimXYZ')
                    X = trialToProcess.m_aiStimXYZ(1);
                    Y = trialToProcess.m_aiStimXYZ(2);
                    Z = trialToProcess.m_aiStimXYZ(3);
                    
                else
                    X = trialToProcess.m_iXCoordinate(trialToProcess.m_iCurrentlySelectedDKLCoordinateID);
                    Y =	trialToProcess.m_iYCoordinate(trialToProcess.m_iCurrentlySelectedDKLCoordinateID);
                    Z =	trialToProcess.m_iZCoordinate;
            ...
            [g_strctParadigm.m_strctMasterColorTable.(g_strctParadigm.m_acstrSaturationsLookup{iSaturations}).az(iColors), ...
             ...
             g_strctParadigm.m_strctMasterColorTable.(g_strctParadigm.m_acstrSaturationsLookup{iSaturations}).el(iColors) = ...
                ...
                %}
				if isfield(trialToProcess,'m_afSphereCoordinates')
					az = trialToProcess.m_afSphereCoordinates(1);
                    el = trialToProcess.m_afSphereCoordinates(2);
                    rad = trialToProcess.m_afSphereCoordinates(3);
                ...
                %rad = trialToProcess.dklRadius;
                %el = trialToProcess.dklElevation
                %az =
...
global g_strctStatistics unitsInUpdate
                ...
                elseif isfield(trialToProcess,'m_aiStimXYZ')
                    ...
                    X = trialToProcess.m_iXCoordinate(trialToProcess.m_iCurrentlySelectedLUVCoordinateID);
                    Y =	trialToProcess.m_iYCoordinate(trialToProcess.m_iCurrentlySelectedLUVCoordinateID);
                    Z =	trialToProcess.m_iZCoordinate;
            ...
            [g_strctParadigm.m_strctMasterColorTable.(g_strctParadigm.m_acstrSaturationsLookup{iSaturations}).az(iColors), ...
             ...
             g_strctParadigm.m_strctMasterColorTable.(g_strctParadigm.m_acstrSaturationsLookup{iSaturations}).el(iColors) = ...
                ...
                %rad = trialToProcess.LUVRadius;
                %el = trialToProcess.LUVElevation
                %az =
