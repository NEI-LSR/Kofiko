    ...
    if isfield(exptDataK{iTrials,2},'m_bMonkeyFixated') && exptDataK{iTrials,2}.m_bMonkeyFixated
        ...
        for iUnit = 1:numUnitsInUpdate
           ...
           if isfield(exptDataK{iTrials, 2}, 'm_strImageColorSpace') && strcmp(exptDataK{iTrials, 2}.m_strImageColorSpace, 'LUV')
                fnProcessLUVTrial(iTrials, unitsInUpdate,iUnit,spikesInThisTrial)
           else
               fnProcessDKLTrial(iTrials, unitsInUpdate,iUnit,spikesInThisTrial)
           end
            ...
            if isfield(exptDataK{iTrials,2},'m_aiStimxyY')
                ...
                %roundedtrackedDKLCoordinates = myRoundn(g_strctStatistics.trackedDKLCoordinates,-3);
                ...
                %{
					colorIDX = ismember(myRoundn(strctColorValues.allDKLCoordinates(:,1),-3),roundedThisTrialDKL(:,1)) &...
                        ismember(myRoundn(strctColorValues.allDKLCoordinates(:,2),-3),roundedThisTrialDKL(:,2))&...
                        ismember(myRoundn(strctColorValues.allDKLCoordinates(:,3),-3),roundedThisTrialDKL(:,3));
                %}
                ...
                if any(allColorIDX)
                    ...
                    %{
						  	allColorIDX = ismember(myRoundn(strctColorValues.allDKLCoordinates(:,1),-3),roundedThisTrialDKL(:,1)) &...
						   ismember(myRoundn(strctColorValues.allDKLCoordinates(:,2),-3),roundedThisTrialDKL(:,2))&...
						   ismember(myRoundn(strctColorValues.allDKLCoordinates(:,3),-3),roundedThisTrialDKL(:,3));
                    %}
                    
                    
                    % g_strctStatistics.trackedDKLCoordinates = myRoundn(strctColorValues.allDKLCoordinates(allColorIDX,:),-3);
...
%{
numCurrentlyTrackedColorUnits = max([max(find(max(max(g_strctStatistics.m_aiDKLCoorindateSpikeHolder)))),numUnitsInUpdate]);
numCurrentlyTrackedOrientationUnits = max([max(find(max(max(g_strctStatistics.m_aiOrientationSpikeHolder)))),numUnitsInUpdate]);
numCurrentlyTrackedXYZUnits = max([max(find(max(max(g_strctStatistics.m_aiDKLCoorindateSpikeHolder)))),numUnitsInUpdate]);
...
%{
if ~any(any(g_strctStatistics.trackedDKLCoordinates)) && ~any(numCurrentlyTrackedOrientationUnits) && ~any(any(g_strctStatistics.trackedDKLCoordinates))
    return;
    
end
%}
PSTHOrientationHolder = [];
if ~isempty(g_strctStatistics.trackedDKLCoordinates)
...
if ~isempty(g_strctStatistics.trackedLUVCoordinates)
...
if isempty(activeDKLConditionsIDX)
    activeDKLConditionsIDX = 0
end
if isempty(activeConditionsDKLIDX)
activeConditionsDKLIDX = 0;
end
%}
%{
...
g_strctStatistics.m_aiDKLPSTHColorHolder = zeros(sum([numel(activeDKLConditionsIDX),numel(activeConditionsDKLIDX)]),50,max(unitsInUpdate));
g_strctStatistics.m_aiLUVPSTHColorHolder = zeros(sum([numel(activeDKLConditionsIDX),numel(activeConditionsDKLIDX)]),50,max(unitsInUpdate));
...
for iUnit = 1:numUnitsInUpdate
    ...
    if g_strctStatistics.m_strctEnabledPlots.m_bPSTHEnabled && ~isempty(g_strctStatistics.trackedDKLCoordinates)
        ...
        for iDKLConditions = 1:numel(activeConditionsDKLIDX)
            ...
            g_strctStatistics.m_aiDKLPSTHColorHolder(iDKLConditions+numel(activeDKLConditionsIDX),:,unitsInUpdate(iUnit)) = ...
                ...
                g_strctStatistics.m_aiDKLPSTHColorHolder(iDKLConditions + numel(activeDKLConditionsIDX),:,unitsInUpdate(iUnit)) = g_strctStatistics.m_aiDKLPSTHColorHolder(iDKLConditions+numel(activeDKLConditionsIDX), :, unitsInUpdate(iUnit)) /...
 ...
 subplot(g_strctStatistics.numEnabledPlots,g_strctStatistics.numCurrentlyTrackedUnits*10,thisSubplotStartingIndex);
            ...
            [colorBar] = fnMakeColorBar(vertcat(g_strctStatistics.m_aiDKLCoorindateColorMatchingTable(find(g_strctStatistics.m_aiDKLCoorindateColorMatchingTable(:,1)),:),...
    ...
    if g_strctStatistics.m_strctEnabledPlots.m_bLUVPSTHEnabled && ~isempty(g_strctStatistics.trackedLUVCoordinates)
        ...
        for iLUVConditions = 1:numel(activeConditionsLUVIDX)
            ...
            g_strctStatistics.m_aiLUVPSTHColorHolder(iLUVConditions+numel(activeLUVConditionsIDX),:,unitsInUpdate(iUnit)) = ...
                ...
                g_strctStatistics.m_aiLUVPSTHColorHolder(iLUVConditions + numel(activeLUVConditionsIDX),:,unitsInUpdate(iUnit)) = g_strctStatistics.m_aiLUVPSTHColorHolder(iLUVConditions+numel(activeLUVConditionsIDX), :, unitsInUpdate(iUnit)) /...
       ...
       thisSubplotStartingIndex = ((((plotRowInUse-1) * 10) * max(unitsInUpdate)  + (unitsInUpdate(iUnit)-1)*10) + 1);
        ...
        %{
           ...
           subplot(g_strctStatistics.numEnabledPlots,g_strctStatistics.numCurrentlyTrackedUnits*10,thisSubplotStartingIndex);
            ...
            [colorBar] = fnMakeColorBar(vertcat(g_strctStatistics.m_aiLUVCoorindateColorMatchingTable(find(g_strctStatistics.m_aiLUVCoorindateColorMatchingTable(:,1)),:),...
   ...
   % dbstop if warning
    ...
    if g_strctStatistics.m_strctEnabledPlots.m_bImagePlotEnabled && isfield(g_strctStatistics,'m_strctImageAnalysis') && ~isempty(g_strctStatistics.m_strctImageAnalysis)
        ...
        TrialTimeTemplate = linspace(g_strctStatistics.preTrialWindow,g_strctStatistics.curTrialWindow,50);
		%{
        g_strctStatistics.m_strctImageAnalysis.m_aiSpikeHolder(nameID,g_strctStatistics.m_strctImageAnalysis.m_aiSpikeHolderIDX : ...
									g_strctStatistics.m_strctImageAnalysis.m_aiSpikeHolderIDX + numel(spikesInThisTrial) -1, unitsInUpdate(iUnit)) = spikesInThisTrial;
       ...
       % for iXYZConditions = 1:numel(activeConditionsDKLIDX)
            % histogram and subtract out our template
        %    g_strctStatistics.PSTHColorHolder(iXYZConditions+numel(activeDKLConditionsIDX),:,unitsInUpdate(iUnit)) = ...
         %       histcounts(tempSpikes(iXYZConditions,1:(g_strctStatistics.m_aiDKLCoorindateSpikeHolderIDX(iXYZConditions, unitsInUpdate(iUnit))-1)+numel(TrialTimeTemplate)),50)-1;
		%	
			tic
...
g_strctStatistics.PSTHImageHolder(iImageCategories,:,unitsInUpdate(iUnit))
        ...
        %{
   subplot(g_strctStatistics.numEnabledPlots,...
   g_strctStatistics.numCurrentlyTrackedUnits*10,...
   (2+((unitsInUpdate(iUnit)-1)*10)+...
            (g_strctStatistics.numCurrentlyTrackedUnits *...
            (g_strctStatistics.numEnabledPlots-g_strctStatistics.numAvailableSubplotRows)))...
            :(unitsInUpdate(iUnit)*10 + (g_strctStatistics.numCurrentlyTrackedUnits *...
            (g_strctStatistics.numEnabledPlots-g_strctStatistics.numAvailableSubplotRows))));
		...
		%text(0,iNumCurrentlyTrackedImageCategories + (1/iNumCurrentlyTrackedImageCategories*.5), ...
        %{
		text(10 ,bottom + (iImageCategories * (height/ iNumCurrentlyTrackedImageCategories)) - (.5 * (height/ iNumCurrentlyTrackedImageCategories))*10 , ...
				g_strctStatistics.m_strctImageAnalysis.m_cstrImageNames{iImageCategories},'interpreter','none', 'color', [1,0,0], 'FontSize',20);
%}
end
%{
			set(gca,'xTickLabel','')
...
if isfield(trialToProcess,'m_strImageFileCategory') && isfield(trialToProcess,'m_strImageName') && isfield(trialToProcess,'m_iImageNameID')
    ...
    %{
    if ismember(g_strctStatistics.m_strctImageAnalysis.m_cstrImageCategories,trialToProcess.m_strImageFileCategory)
		categoryID = find(ismember(g_strctStatistics.m_strctImageAnalysis.m_cstrImageCategories,trialToProcess.m_strImageFileCategory));

		g_strctStatistics.m_strctImageAnalysis.m_aiImageCategoryTrialCounter(categoryID) = ...
			g_strctStatistics.m_strctImageAnalysis.m_aiImageCategoryTrialCounter(categoryID) + 1;
        
	else
		g_strctStatistics.m_strctImageAnalysis.m_cstrImageCategories{end + 1} = trialToProcess.m_strImageFileCategory;
        categoryID = size(g_strctStatistics.m_strctImageAnalysis.m_cstrImageCategories,1);
		g_strctStatistics.m_strctImageAnalysis.m_aiImageCategoryTrialCounter(end+1) = 1;
    end
    %}
...
%{
        maxVal = max(g_strctStatistics.m_aiDKLCoorindateSpikeHolder(activeConditionsDKLIDX(iXYZConditions),g_strctStatistics.m_aiDKLCoorindateSpikeHolder(activeConditionsDKLIDX(iXYZConditions),...
            :,unitsInUpdate(iUnit)) ~= 0));
        
        minVal = min(	g_strctStatistics.m_aiDKLCoorindateSpikeHolder(activeConditionsDKLIDX(iXYZConditions),g_strctStatistics.m_aiDKLCoorindateSpikeHolder(activeConditionsDKLIDX(iXYZConditions),...
            :,unitsInUpdate(iUnit)) ~= 0));
%}
%{
            if  g_strctStatistics.curTrialWindow < maxVal
...
%{
            bleh = zeros(
            for izTest = 1:numel(activeConditionsDKLIDX)
                bleh
                
            end
            
            funFunc = @(x) x
            tempSpikes1 = arrayfun(@cat, (g_strctStatistics.m_aiDKLCoorindateSpikeHolder(activeConditionsDKLIDX,:, unitsInUpdate(iUnit))),repmat(TrialTimeTemplate,[numel(activeConditionsDKLIDX),1]),2)
            tempSpikes1 = arrayfun(@(x) cat(2,g_strctStatistics.m_aiDKLCoorindateSpikeHolder(activeConditionsDKLIDX,:, unitsInUpdate(iUnit)),repmat(TrialTimeTemplate,[numel(activeConditionsDKLIDX),1])))
           tempSpikes2 = bsxfun(@(x) histcounts(cat(2,g_strctStatistics.m_aiDKLCoorindateSpikeHolder(x,:, unitsInUpdate(iUnit)),repmat(TrialTimeTemplate,[numel(activeConditionsDKLIDX),1]))))
                       tempSpikes2 = bsxfun(@(x) histcounts(cat(2,g_strctStatistics.m_aiDKLCoorindateSpikeHolder(activeConditionsDKLIDX,:, unitsInUpdate(iUnit)),repmat(TrialTimeTemplate,[numel(activeConditionsDKLIDX),1]))))

            
            
            tempSpikes1 = arrayfun(@horzcat, g_strctStatistics.m_aiDKLCoorindateSpikeHolder(activeConditionsDKLIDX,g_strctStatistics.m_aiDKLCoorindateSpikeHolder(activeConditionsDKLIDX,...
                :,unitsInUpdate(iUnit)) ~= 0, unitsInUpdate(iUnit)),TrialTimeTemplate)
            
            tempSpikes = [g_strctStatistics.m_aiDKLCoorindateSpikeHolder(activeConditionsDKLIDX(iXYZConditions),g_strctStatistics.m_aiDKLCoorindateSpikeHolder(activeConditionsDKLIDX(iXYZConditions),...
                :,unitsInUpdate(iUnit)) ~= 0, unitsInUpdate(iUnit)),TrialTimeTemplate];
            
             tempSpikes = [g_strctStatistics.m_aiDKLCoorindateSpikeHolder(activeConditionsDKLIDX,g_strctStatistics.m_aiDKLCoorindateSpikeHolder(activeConditionsDKLIDX,...
                :,unitsInUpdate(iUnit)) ~= 0, unitsInUpdate(iUnit)),TrialTimeTemplate];
%}
...
function fnProcessDKLTrial(iTrials, unitsInUpdate,  iUnit,spikesInThisTrial)
global g_strctStatistics exptDataK
if ~isfield(exptDataK{iTrials,2},'m_aiStimxyY') && isfield(exptDataK{iTrials,2},'m_iXCoordinate') || isfield(exptDataK{iTrials,2},'m_aiStimXYZ') && ~any(isnan(exptDataK{iTrials,2}.m_aiStimXYZ))
            ...
            [g_strctParadigm.m_strctMasterColorTable.(g_strctParadigm.m_acstrSaturationsLookup{iSaturations}).az(iColors), ...
             ...
             g_strctParadigm.m_strctMasterColorTable.(g_strctParadigm.m_acstrSaturationsLookup{iSaturations}).el(iColors) = ...
                ...
                theta(iColors) = tan(g_strctParadigm.m_strctMasterColorTable.(g_strctParadigm.m_acstrSaturationsLookup{iSaturations}).XYZ(iColors,3) ...
                    ...
                    rad(iColors) = cos(theta(iColors))*sqrt(g_strctParadigm.m_strctMasterColorTable.(g_strctParadigm.m_acstrSaturationsLookup{iSaturations}).XYZ(iColors,1)^2 ...
                        + g_strctParadigm.m_strctMasterColorTable.(g_strctParadigm.m_acstrSaturationsLookup{iSaturations}).XYZ(iColors,2)^2 ... 
                ...
                %}
				if isfield(exptDataK{iTrials,2},'m_afSphereCoordinates')
					az = exptDataK{iTrials,2}.m_afSphereCoordinates(1);
                    el = exptDataK{iTrials,2}.m_afSphereCoordinates(2);
                    rad = exptDataK{iTrials,2}.m_afSphereCoordinates(3);
                ...
                if isempty(g_strctStatistics.trackedDKLCoordinates) || ~any(ismember(g_strctStatistics.trackedDKLCoordinates,myRoundn([az,el,rad],-2),'rows'))
                    ...
                    if ~exptDataK{iTrials,2}.m_bFlipForegroundBackground
                        if size(exptDataK{iTrials,2}.m_aiStimColor,1) > 1
                            ...
                            g_strctStatistics.m_aiDKLCoorindateColorMatchingTable = vertcat(g_strctStatistics.m_aiDKLCoorindateColorMatchingTable,[myRoundn([az,el,rad],-2), ...
							exptDataK{iTrials,2}.m_aiStimColor]);
                        end
                    else
                        if size(exptDataK{iTrials,2}.m_afBackgroundColor,1) > 1
                            ...
                            g_strctStatistics.m_aiDKLCoorindateColorMatchingTable = ...
                                vertcat(g_strctStatistics.m_aiDKLCoorindateColorMatchingTable,[myRoundn([az,el,rad],-2),exptDataK{iTrials,2}.m_afBackgroundColor]);
                        end
                    end
                    g_strctStatistics.trackedDKLCoordinates = vertcat(g_strctStatistics.trackedDKLCoordinates,myRoundn([az,el,rad],-2));
                    %uniquetrackedDKLCoordinates = myRoundn(unique(g_strctStatistics.trackedDKLCoordinates,'rows'),-2);
                   ...
                   % [uniqueRadii] = unique(uniquetrackedDKLCoordinates(:,3));
                    ...
                    for iRadii = 1 : numel(uniqueRadii)
                        ...
                        %uniqueEl = unique(uniquetrackedDKLCoordinates(thisRadiusIDX,2));
...
function fnProcessLUVTrial(iTrials, unitsInUpdate, iUnit, spikesInThisTrial)
global g_strctStatistics exptDataK
                ...
                elseif isfield(exptDataK{iTrials,2},'m_aiStimXYZ')
                    ...
                    X = exptDataK{iTrials,2}.m_iXCoordinate(exptDataK{iTrials,2}.m_iCurrentlySelectedLUVCoordinateID);
                    Y =	exptDataK{iTrials,2}.m_iYCoordinate(exptDataK{iTrials,2}.m_iCurrentlySelectedLUVCoordinateID);
                    Z =	exptDataK{iTrials,2}.m_iZCoordinate;
            ...
            [g_strctParadigm.m_strctMasterColorTable.(g_strctParadigm.m_acstrSaturationsLookup{iSaturations}).az(iColors), ...
             ...
             g_strctParadigm.m_strctMasterColorTable.(g_strctParadigm.m_acstrSaturationsLookup{iSaturations}).el(iColors) = ...
                ...
                %rad = exptDataK{iTrials,2}.LUVRadius;
                %el = exptDataK{iTrials,2}.LUVElevation
                %az =
                if isempty(g_strctStatistics.trackedLUVCoordinates) || ~any(ismember(g_strctStatistics.trackedLUVCoordinates,myRoundn([az,el,rad],-2),'rows'))
                    ...
                    if ~exptDataK{iTrials,2}.m_bFlipForegroundBackground
                        ...
                        if size(exptDataK{iTrials,2}.m_afBackgroundColor,1) > 1
                            ...
                            g_strctStatistics.m_aiLUVCoorindateColorMatchingTable = ...
                                vertcat(g_strctStatistics.m_aiLUVCoorindateColorMatchingTable,[myRoundn([az,el,rad],-2),exptDataK{iTrials,2}.m_afBackgroundColor]);
                        end
                    end
                    g_strctStatistics.trackedLUVCoordinates = vertcat(g_strctStatistics.trackedLUVCoordinates,myRoundn([az,el,rad],-2));
                    %uniquetrackedLUVCoordinates = myRoundn(unique(g_strctStatistics.trackedLUVCoordinates,'rows'),-2);
                   ...
                   % [uniqueRadii] = unique(uniquetrackedLUVCoordinates(:,3));
                    ...
                    for iRadii = 1 : numel(uniqueRadii)
                        ...
                        %uniqueEl = unique(uniquetrackedLUVCoordinates(thisRadiusIDX,2));
