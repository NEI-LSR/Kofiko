...
switch lower(strCommand)
    ...
    case 'setdynamictrialmode'
  	...
  	case 'ProcessRingAFCTrial'
		if ~g_strctConfig.m_bRingAFCTaskInitialized
			fnInitializeTask('RingAFC');
		end
        %fnAcceptDynamicTrialInformation(acInputFromKofiko{2});
		fnProcessRingAFCTrial();
        ...
        if ischar(acInputFromKofiko{2})
            ...
            switch(acInputFromKofiko{2})
                case 'flushcolorspikebuffer'
                    ...
                    g_strctStatistics.m_aiDKLCoorindateColorMatchingTable = zeros(200,6);
                ...
                case 'clearluvpsthplot'
                    ...
                    g_strctStatistics.m_aiLUVCoorindateColorMatchingTable = [];%zeros(200,6);
                    g_strctStatistics.m_aiLUVCoorindateColorTrialCounter = zeros(200,5);
                    %g_strctStatistics.trackedLUVCoordinates  = nan(size(strctColorValues.allLUVCoordinates,1),size(strctColorValues.allLUVCoordinates,2));
			...
			for iCmnds = 1:numel(acInputFromKofiko{2})
				switch acInputFromKofiko{2}(iCmnds).cmd{1}
                    ...
                    case 'TogglePSTHPlot'
                        ...
                        g_strctConfig.m_strctEnabledPlots.m_bPSTHEnabled = ~g_strctConfig.m_strctEnabledPlots.m_bPSTHEnabled;
						
					 case 'ToggleLUVPSTHPlot'
						g_strctConfig.m_strctEnabledPlots.m_bLUVPSTHEnabled = ~g_strctConfig.m_strctEnabledPlots.m_bLUVPSTHEnabled;
						
                    case 'ToggleOrientationPolar'
                        g_strctConfig.m_strctEnabledPlots.m_bOrientationPolarEnabled = ~g_strctConfig.m_strctEnabledPlots.m_bOrientationPolarEnabled;

					case 'ToggleVariableLengthPlot'
						g_strctConfig.m_strctEnabledPlots.m_bVariableLengthPlotEnabled = ~g_strctConfig.m_strctEnabledPlots.m_bVariableLengthPlotEnabled;

					case 'ToggleVariableWidthPlot'
						g_strctConfig.m_strctEnabledPlots.m_bVariableWidthPlotEnabled = ~g_strctConfig.m_strctEnabledPlots.m_bVariableWidthPlotEnabled;
						
					case 'ToggleVariableSpeedPlot'
						g_strctConfig.m_strctEnabledPlots.m_bVariableSpeedPlotEnabled = ~g_strctConfig.m_strctEnabledPlots.m_bVariableSpeedPlotEnabled;
						
                    case 'ToggleISIPlot'
                        g_strctConfig.m_strctEnabledPlots.m_bISIPlotEnabled = ~g_strctConfig.m_strctEnabledPlots.m_bISIPlotEnabled;

                    case 'TogglePositionPlot'
                        g_strctConfig.m_strctEnabledPlots.m_bPositionHeatmapEnabled = ~g_strctConfig.m_strctEnabledPlots.m_bPositionHeatmapEnabled;
						
					case 'ToggleImagePlot'
						g_strctConfig.m_strctEnabledPlots.m_bImagePlotEnabled = ~g_strctConfig.m_strctEnabledPlots.m_bImagePlotEnabled;
					...
					case 'PrintFigure'
					try
					set(g_strctWindows.m_hFigureHandle,'paperorientation','landscape');
					print(g_strctWindows.m_hFigureHandle,'resize','-bestfit');
