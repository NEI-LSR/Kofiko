...
%function fnKofikoCycleClean(g_bParadigmRunning)
%
...
while g_bParadigmRunning
    ...
    if g_strctRealTimeStatServer.m_bConnected
        ...
        if ~isempty(acMessageFromRealTimeStatServer)
			...
			elseif strcmpi(strCommand,'sendtrialinfo')
				g_strctRealTimeStatServer.m_bTrialInfoWait = 1;
			
			elseif strcmpi(strCommand,'SendFullTrialBackup')
				g_strctRealTimeStatServer.m_bSendAllTrialsForBackup = true;
			elseif strcmpi(strCommand,'requestsync')
    ...
    if  strcmpi(g_strctDAQParams.m_strEyeSignalInput,'Analog')
        ...
        [afAnalogSignals] = fnDAQWrapper('GetAnalog',[g_strctDAQParams.m_fMotionPort,...
            ...
            %}
            % New style DAQ, reads all 16 AD channels at once and matlab parses the individual channels afterward.
            % This takes ~.3 ms. Old style took 2ms.
            % If you change this you must also change the GetAnalog call in fnRecenterGaze or Matlab might crash.
            % Explanation: the card works on a task system, so each time you read an individual channel it stops
            % the previous task, starts the new task, reads the channel, and moves on. The overhead from starting
            % and stopping tasks is what kills the timing, not the read itself. This could probably be cut down to
            % a very small number, smaller than .3ms, but we need the digital output tasks to run for strobewords
            % and microstim control and some task switching is probably necessary.
            [afAnalogSignals] = fnDAQWrapper('GetAnalog',1:16);
            
            if ~isempty(afAnalogSignals)
                fMotionSignal = afAnalogSignals(g_strctDAQParams.m_fMotionPort+1);
                aiExternalTriggerIndices = 5:5+iNumExternalTriggers-1;
                afRAWEyeSig = afAnalogSignals([g_strctDAQParams.m_fEyePortX,g_strctDAQParams.m_fEyePortY, g_strctDAQParams.m_fEyePortPupil]+1);
                afExternalTriggerValues = afAnalogSignals([g_strctDAQParams.m_afExternalTriggers:g_strctDAQParams.m_afExternalTriggers + iNumExternalTriggers-1]);
                
                %if ~isempty(g_strctCycle.m_aFutureSignals)
                %afCycleDebugTimers(18) = GetSecs();
                %afAnalogSignals = fnDAQWrapper('GetFutureOut');
    ...
    if strcmpi(g_strctDAQParams.m_strEyeSignalInput,'Analog') && ...
        ...
        if gt(fCycleTic - g_strctCycle.m_fEyeTrackTimer, g_strctCycle.m_fEyeTrackElapsedMS/1000)
            iLastEntry = g_strctEyeCalib.EyeRaw.BufferIdx;
    ...
    if g_strctAppConfig.m_strctVarSave.m_fMotion && g_strctCycle.m_bMotionBufferInitialized
        if gt(fCycleTic - g_strctCycle.m_strctMotion.m_fSampleTimer, g_strctCycle.m_strctMotion.m_fElapsedMS/1000)
    ...
    for iMicroStimIter=1:iNumMicroStimChannels
        spikesToTrig = numel(g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_fRequestTrigTS);
        if g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_bActive &&  ...
			...
			if g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_iSpikeIter == 1
			% first pulse, set all the timestamps starting from now
				g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_fRequestTrigTS = ...
				(g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_strctTriggerInformation.m_afSpikeTrain*1e-3) + fCurrTime;
				
				% Also, handle the first pulse here, since we already know it has to happen now
				 fnDAQWrapper('SetBit', g_strctAppConfig.m_strctDAQ.m_afStimulationPort(iMicroStimIter), 1);
				delay(g_strctParadigm.MicroStimPulseWidthMS.Buffer(g_strctParadigm.MicroStimPulseWidthMS.BufferIdx)*1e-6)
				fnDAQWrapper('SetBit', g_strctAppConfig.m_strctDAQ.m_afStimulationPort(iMicroStimIter), 0);
				
				delay(g_strctParadigm.MicroStimBipolarDelayMS.Buffer(g_strctParadigm.MicroStimBipolarDelayMS.BufferIdx)*1e-6)
				
				fnDAQWrapper('SetBit', g_strctAppConfig.m_strctDAQ.m_afStimulationPort(iMicroStimIter), 1);
				delay(g_strctParadigm.MicroStimSecondPulseWidthMS.Buffer(g_strctParadigm.MicroStimSecondPulseWidthMS.BufferIdx)*1e-6)
				fnDAQWrapper('SetBit', g_strctAppConfig.m_strctDAQ.m_afStimulationPort(iMicroStimIter), 0);
				g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_afTriggerTS(...
					g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_iSpikeIter) = fCurrTime;
				g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_iSpikeIter =...
					g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_iSpikeIter + 1;
			...
			% is it almost time to stimulate, but not quite? We may not want to wait another cycle to do it, so lets pause and wait
			
			
            if g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_iSpikeIter > spikesToTrig
                % we're done here, no more stim
			...
			timeToNextPulse = fCurrTime - g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_fRequestTrigTS(...
                    g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_iSpikeIter)
					timeToNextPulse > -2 * 1e-3 && timeToNextPulse < 0
					
			if 	timeToNextPulse > -2 * 1e-3 && timeToNextPulse < 0
					WaitSecs(abs(timeToNextPulse))
			end
			timeToNextPulse > -2 * 1e-3 && timeToNextPulse < 0
					%{
			while timeToNextPulse < 2 * 1e-3 && timeToNextPulse > 0
				timeToNextPulse = fCurrTime - g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_fRequestTrigTS(...
                    g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_iSpikeIter);
			end
			%}
            if fCurrTime >= g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_fRequestTrigTS(...
                    g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_iSpikeIter)
					disp('pulse')
                % more than 1 spike in this train, we'll need to trigger each time
        ...
        elseif g_strctCycle.m_strctMicroStim.m_astrctTriggeringMachines(iMicroStimIter).m_bActive  && ...
    ...
    if g_strctGUIParams.m_bUseSpecialKeys
        if keyIsDown && g_strctCycle.m_iKeyFSM == 0
			fnHandleKeyPressEvents(keyCode, keyIsDown);
    ...
    if (fCurrTime-g_strctCycle.m_fScreenTimer) > g_strctGUIParams.m_fRefreshRateMS / 1000 && ...
       ...
       % end
		...
		if isfield(g_strctParadigm, 'm_strctEyeSmoothing') && isfield( g_strctParadigm.m_strctEyeSmoothing,'m_bEyeSmoothingEnabled') && g_strctParadigm.m_strctEyeSmoothing.m_bEyeSmoothingEnabled
			meanEyeX = mean(g_strctParadigm.m_strctEyeSmoothing.m_afEyeSmoothingArray(:,1));
			meanEyeY = mean(g_strctParadigm.m_strctEyeSmoothing.m_afEyeSmoothingArray(:,2));
			aiFixationRect = g_strctPTB.m_fScale*[meanEyeX,meanEyeY,meanEyeX,meanEyeY] + [-5 -5 5 5];
		else
		...
		g_strctParadigm.m_bFlipJustOccurred = true;
        fDrawToc1 = GetSecs;
