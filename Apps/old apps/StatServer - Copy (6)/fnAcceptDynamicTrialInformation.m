...
for iTrials = 1:size(exptDataK,1)
    ...
    if isfield(exptDataK{iTrials,2},'m_bMonkeyFixated') && exptDataK{iTrials,2}.m_bMonkeyFixated
        g_strctStatistics.curTrialWindow = exptDataK{iTrials,2}.m_iTrialLength_MS/1000;
...
g_strctStatistics.PSTHColorHolder = zeros(size(activeConditionsIDX,1),50,numCurrentlyTrackedUnits);
...
for iUnit = 1:numUnitsInUpdate
    ...
    if any(maxVal) & any(minVal)
        if g_strctStatistics.curTrialWindow >= maxVal && -preTrialWindow <=  minVal
										
	
            TrialTimeTemplate = linspace(-preTrialWindow,g_strctStatistics.curTrialWindow,50);
