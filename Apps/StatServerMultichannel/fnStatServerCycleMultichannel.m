function fnStatServerCycleMultichannel()
global g_strctCycle g_strctConfig g_strctNet g_strctNeuralServer g_strctWindows g_DebugDataLog g_counter g_strctStatistics g_strctTrial
...
if g_strctCycle.m_bClientConnected
        ...
        if sum(g_strctNeuralServer.m_a2iCurrentActiveUnits(:)>0) > 0
           ...
           if ~strcmpi(strAnswer,'No')
               ...
               for k=1:length(aiInd)
                    fnStatServerCallbacksMultichannel('ToggleActive',aiChannel(k),aiUnit(k));
               end
           end
        end
    ...
    if ~isempty(acInputFromKofiko) && iscell(acInputFromKofiko) && length(acInputFromKofiko) >= 1 && g_strctNeuralServer.m_bConnected
        fnStatServerCallbacksMultichannel('KofikoInput',acInputFromKofiko);
    end
end


if g_strctCycle.m_bDynamicTrialMode
	if g_strctCycle.m_bClientConnected && g_strctCycle.m_bDynamicTrialMode && fCurrTime - g_strctCycle.m_iLastStatServerUpdateTS > g_strctCycle.m_iStatServerUpdateInterval 
		mssend(g_strctNet.m_iCommSocket, {'requestsync'});
        mssend(g_strctNet.m_iCommSocket, {'sendtrialinfo'});
        

		[acInputFromKofiko, iSocketErrorCode] = msrecv(g_strctNet.m_iCommSocket, .3);
		if ~isempty(acInputFromKofiko)
			g_strctCycle.m_iLastStatServerUpdateTS = fCurrTime;
			fnStatServerCallbacksMultichannel('KofikoInput', acInputFromKofiko);
        end
