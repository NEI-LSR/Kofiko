    ...
    if isempty(strctTrial.m_strImageName) 
     ...
     try
        ...
        elseif strcmp(strctTrial.m_strImageColorSpace, 'LUV') & numel(strComponents) == 4 || numel(strComponents) == 5 
          azimuthInfoIDX = regexp(strComponents(2),'\d');
            azimuthInfoString = strComponents(2);
            strctTrial.azimuthInfo = str2double(azimuthInfoString{1}(azimuthInfoIDX{1}));
            
            elevationInfoIDX = regexp(strComponents(4),'\d');
            elevationInfoString = strComponents(4);
            strctTrial.elevationInfo = str2double(elevationInfoString{1}(elevationInfoIDX{1}));
            if strcmp(elevationInfoString{1}(elevationInfoIDX{1}(1)-1), '-')
            ...
            saturationInfoIDX = regexp(strComponents(3),'\d');
            saturationInfoString = strComponents(3);
            strctTrial.saturationInfo = str2double(saturationInfoString{1}(saturationInfoIDX{1}));
            % [x, y, z] = sph2cart(deg2rad(azimuthInfo),elevationInfo/100,saturationInfo/100);
            % strctTrial.m_aiStimXYZ = [x,y,z]
            
            % L coordinate is Z in cartesian space, so order is 2, 3, 1
            [strctTrial.m_aiStimLUV(2), strctTrial.m_aiStimLUV(3), strctTrial.m_aiStimLUV(1)] = sph2cart(deg2rad(strctTrial.azimuthInfo),0,strctTrial.saturationInfo);
            strctTrial.m_aiStimLUV(1) = strctTrial.elevationInfo;
