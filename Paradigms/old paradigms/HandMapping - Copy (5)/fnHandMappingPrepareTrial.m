...
elseif g_strctParadigm.m_bUseCartesianCoordinates
	if ~g_strctParadigm.m_bCycleColors 
		...
		dklRadius = squeeze(g_strctParadigm.DKLRadius.Buffer(1,:,g_strctParadigm.DKLRadius.BufferIdx));
		dklElevation = squeeze(g_strctParadigm.DKLElevation.Buffer(1,:,g_strctParadigm.DKLElevation.BufferIdx));
		dklAzimuthSteps = squeeze(g_strctParadigm.DKLAzimuthSteps.Buffer(1,:,g_strctParadigm.DKLAzimuthSteps.BufferIdx));
		if dklAzimuthSteps < 1
			dklAzimuthSteps = 1;
		end
		
		ai_dklAzimuthSteps = linspace(1,360,dklAzimuthSteps);
		[x,y,z] = sph2cart(deg2rad(ai_dklAzimuthSteps),(dklElevation/100)-1,dklRadius/100);
        for i = 1:numel(x)
			FullCycleRGB(i,:) = ldrgyv2rgb(z,x(i),y(i));
        end
        FullCycleRGB(FullCycleRGB >= 1) = 1;
		FullCycleRGB(FullCycleRGB <= 0) = 0;
		
		
		if ~g_strctParadigm.m_bReverseColorOrder
		...
		if g_strctParadigm.m_bPhaseShiftOffset
			ShiftOffset = squeeze(g_strctParadigm.ShiftOffsetDegrees.Buffer(1,:,g_strctParadigm.ShiftOffsetDegrees.BufferIdx));
			[x,y,z] = sph2cart(deg2rad(ShiftOffset)+deg2rad(ai_dklAzimuthSteps(g_strctParadigm.m_iCurrentlySelectedDKLCoordinateID)),(dklElevation/100)-1,dklRadius/100);
