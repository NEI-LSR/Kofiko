...
% Get our master color table in case we do a color tuning function

g_strctParadigm.m_bDoNotDrawDueToCriticalSection = 0; % because it's in kofiko's cycle now, 
													  %and we might conceivably want the paradigm to be able to use it
...
if ~isempty(g_strctParadigm.m_strInitial_DefaultImageList) && exist(g_strctParadigm.m_strInitial_DefaultImageList,'file')
   if fnLoadHandMappingDesign(g_strctParadigm.m_strInitial_DefaultImageList) % short circuited to return true, we don't need no design file
