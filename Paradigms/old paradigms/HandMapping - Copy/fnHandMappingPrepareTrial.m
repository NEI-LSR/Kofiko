...
elseif g_strctParadigm.m_bUseCartesianCoordinates
	...
	if ~g_strctParadigm.m_bCycleColors && ~g_strctParadigm.m_bRandomColor
		...
		%[x,y,z] = sph2cart(deg2rad(strctTrial.ai_dklAzimuthSteps),(strctTrial.dklElevation/100)-1,strctTrial.dklRadius/100);
		% sin theta plus pi/2
		
		if g_strctParadigm.m_bUseGaussianPulses
			%[x,y,z] = sph2cart(deg2rad(strctTrial.ai_dklAzimuthSteps),deg2rad(strctTrial.dklElevationSteps),strctTrial.dklRadius/100);
			if g_strctParadigm.m_iCurrentlySelectedDKLCoordinateID > numel(strctTrial.ai_dklAzimuthSteps)
				g_strctParadigm.m_iCurrentlySelectedDKLCoordinateID = 1;
			elseif g_strctParadigm.m_iCurrentlySelectedDKLCoordinateID < 1
				g_strctParadigm.m_iCurrentlySelectedDKLCoordinateID = numel(strctTrial.ai_dklAzimuthSteps);
			end
			[x,y,z] = sph2cart(deg2rad(strctTrial.ai_dklAzimuthSteps(g_strctParadigm.m_iCurrentlySelectedDKLCoordinateID)),...
					ones(1,strctTrial.numFrames)*strctTrial.dklElevation,sin(linspace(0,pi,strctTrial.numFrames))*strctTrial.dklRadius/100);
		...
		%[x,y,z] = sph2cart(0,0,sin(linspace(0,2*pi,60)+.5));
		%[x,y,z] = sph2cart(zeros(1,60*5), zeros(1,60*5), max(0,sin(linspace(0,pi*2,60*5))*strctTrial.dklRadius/100));
		%[x,y,z] = sph2cart(zeros(1,60*5), zeros(1,60*5), max(0,sin(linspace(0,pi*2,60*5))*strctTrial.dklRadius/100));

        for i = 1:numel(x)
			FullCycleRGB(i,:) = ldrgyv2rgb(z(i),x(i),y(i));
            %theta = deg2rad(40);
		...
		if g_strctParadigm.m_bUseGaussianPulses
			for iFrames = 1:strctTrial.numFrames
				strctTrial.m_aiStimColor(iFrames,:) = [g_strctParadigm.m_strctGammaCorrectedLookupTable.RLUT(round(FullCycleRGB(iFrames, 1) * 65535) + 1),...
					g_strctParadigm.m_strctGammaCorrectedLookupTable.GLUT(round(FullCycleRGB(iFrames, 2) * 65535) + 1),...
					g_strctParadigm.m_strctGammaCorrectedLookupTable.BLUT(round(FullCycleRGB(iFrames, 3) * 65535) + 1)];
				strctTrial.m_aiLocalStimColor(iFrames,:) = round((strctTrial.m_aiStimColor(iFrames,:)/65535) * 255);
			end
		else
		
			strctTrial.m_aiStimColor = [g_strctParadigm.m_strctGammaCorrectedLookupTable.RLUT(round(FullCycleRGB(g_strctParadigm.m_iCurrentlySelectedDKLCoordinateID, 1) * 65535) + 1),...
					g_strctParadigm.m_strctGammaCorrectedLookupTable.GLUT(round(FullCycleRGB(g_strctParadigm.m_iCurrentlySelectedDKLCoordinateID, 2) * 65535) + 1),...
					g_strctParadigm.m_strctGammaCorrectedLookupTable.BLUT(round(FullCycleRGB(g_strctParadigm.m_iCurrentlySelectedDKLCoordinateID, 3) * 65535) + 1)];
		...
		if g_strctParadigm.m_bPhaseShiftOffset
			...
			[x,y,z] = sph2cart(deg2rad(strctTrial.m_iShiftOffset)+...
			...
			shiftOffsetRGB = ldrgyv2rgb(z,x,y);
			...
			strctTrial.m_afBackgroundColor = [g_strctParadigm.m_strctGammaCorrectedLookupTable.RLUT(round(shiftOffsetRGB(1) * 65535) + 1),...
				g_strctParadigm.m_strctGammaCorrectedLookupTable.GLUT(round(shiftOffsetRGB(2) * 65535) + 1),...
				g_strctParadigm.m_strctGammaCorrectedLookupTable.BLUT(round(shiftOffsetRGB(3) * 65535) + 1)];
           ...
           % strctTrial.m_afLocalBackgroundColor = round((strctTrial.m_afBackgroundColor/65535)*255);
			...
			strctTrial.m_afLocalBackgroundColor = round((strctTrial.m_afBackgroundColor/65535)*255);
		if g_strctParadigm.m_bFlipForegroundBackground
...
elseif g_strctParadigm.m_bCycleColors || g_strctParadigm.m_bRandomColor
	...
	switch strctTrial.m_strTrialType
		case 'Moving Bar'
			if g_strctParadigm.m_bFlipForegroundBackgroundMovingBar
...
strctTrial.m_aiBlurStepHolder = zeros(3,strctTrial.numberBlurSteps,strctTrial.numFrames);
	for iFrames = 1:strctTrial.numFrames
		strctTrial.m_aiLocalBlurStepHolder(1,:,iFrames) = round(linspace(strctTrial.m_afLocalBackgroundColor(1),strctTrial.m_aiLocalStimColor(1),strctTrial.numberBlurSteps));
