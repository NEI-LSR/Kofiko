...
global g_strctParadigm g_strctPTB g_strctAcquisitionServer g_strctNeuralServer g_strctLocalExperimentRecording g_bRecording
...
global g_strctParadigm g_strctLocalExperimentRecording g_strctRealTimeStatServer g_bRecording
...
if ~isempty(g_strctParadigm.m_strctCurrentTrial.m_fImageFlipON_TS_StimulusServer) &&...
        ~isempty(g_strctParadigm.m_strctCurrentTrial.m_fImageFlipON_TS_Kofiko)
		 g_strctParadigm.m_strctCurrentTrial.m_iTrialLength_MS = g_strctParadigm.m_iTrialLength_MS;
    strctTrialBackup = g_strctParadigm.m_strctCurrentTrial;
	%strctTrialBackup
	
	%{
	strctTrialBackup = [];
    strctTrialBackup.m_strTrialType = g_strctParadigm.m_strctCurrentTrial.m_strTrialType;
    strctTrialBackup.m_iTrialLength_MS = g_strctParadigm.m_iTrialLength_MS;
    
    strctTrialBackup.m_fImageFlipON_TS_Kofiko = g_strctParadigm.m_strctCurrentTrial.m_fImageFlipON_TS_Kofiko;
    strctTrialBackup.m_aiStimulusRect = g_strctParadigm.m_aiStimulusRect;
    strctTrialBackup.m_pt2iFixationSpot = g_strctParadigm.m_strctCurrentTrial.m_pt2iFixationSpot;
    
    strctTrialBackup.m_fFixationSizePix = g_strctParadigm.m_strctCurrentTrial.m_fFixationSizePix;
    strctTrialBackup.m_fGazeBoxPix = g_strctParadigm.m_strctCurrentTrial.m_fGazeBoxPix;
    
    
    if strcmpi(strctTrialBackup.m_strTrialType,'plain bar') ||...
            strcmpi(strctTrialBackup.m_strTrialType,'moving bar')
        strctTrialBackup.m_iFixationColor = g_strctParadigm.m_strctCurrentTrial.m_iFixationColor;
        
        strctTrialBackup.m_iLength = g_strctParadigm.m_strctCurrentTrial.m_iLength ;
        strctTrialBackup.m_iWidth = g_strctParadigm.m_strctCurrentTrial.m_iWidth ;
        strctTrialBackup.m_fRotationAngle = g_strctParadigm.m_strctCurrentTrial.m_fRotationAngle ;
        strctTrialBackup.numberBlurSteps = g_strctParadigm.m_strctCurrentTrial.numberBlurSteps ;
        strctTrialBackup.m_bBlur = g_strctParadigm.m_strctCurrentTrial.m_bBlur  ;
        strctTrialBackup.m_fStimulusON_MS = g_strctParadigm.m_strctCurrentTrial.m_fStimulusON_MS ;
        strctTrialBackup.m_fStimulusOFF_MS = g_strctParadigm.m_strctCurrentTrial.m_fStimulusOFF_MS ;
        strctTrialBackup.numFrames = g_strctParadigm.m_strctCurrentTrial.numFrames ;
        strctTrialBackup.m_iNumberOfBars = g_strctParadigm.m_strctCurrentTrial.m_iNumberOfBars ;
        
        
        strctTrialBackup.m_iMoveDistance = g_strctParadigm.m_strctCurrentTrial.m_iMoveDistance ;
        
        strctTrialBackup.location_x = g_strctParadigm.m_strctCurrentTrial.location_x(1);
        strctTrialBackup.location_y = g_strctParadigm.m_strctCurrentTrial.location_y(1);
        
        strctTrialBackup.bar_rect = g_strctParadigm.m_strctCurrentTrial.bar_rect;
        
        strctTrialBackup.m_aiStimColor = g_strctParadigm.m_strctCurrentTrial.m_aiStimColor;
        if isfield(g_strctParadigm.m_strctCurrentTrial,'m_aiStimxyY')
            strctTrialBackup.m_aiStimxyY = g_strctParadigm.m_strctCurrentTrial.m_aiStimxyY;
        end
        if isfield(g_strctParadigm.m_strctCurrentTrial,'m_iSelectedColorIndex')
            strctTrialBackup.m_iSelectedColorIndex = g_strctParadigm.m_strctCurrentTrial.m_iSelectedColorIndex;
        end
        if isfield(g_strctParadigm.m_strctCurrentTrial,'m_iOrientationBin')
            strctTrialBackup.m_iOrientationBin = g_strctParadigm.m_strctCurrentTrial.m_iOrientationBin;
        end
        if isfield(g_strctParadigm.m_strctCurrentTrial,'m_iSelectedSaturationIndex')
            strctTrialBackup.m_iSelectedSaturationIndex = g_strctParadigm.m_strctCurrentTrial.m_iSelectedSaturationIndex;
        end
        if isfield(g_strctParadigm.m_strctCurrentTrial,'m_bMonkeyFixated')
            strctTrialBackup.m_bMonkeyFixated = g_strctParadigm.m_strctCurrentTrial.m_bMonkeyFixated;
        end
        strctTrialBackup.m_afBackgroundColor = g_strctParadigm.m_strctCurrentTrial.m_afBackgroundColor;
        
        
    elseif strcmpi(strctTrialBackup.m_strTrialType,'moving dots')
        
        strctTrialBackup.m_iFixationColor = g_strctParadigm.m_strctCurrentTrial.m_iFixationColor;
        strctTrialBackup.m_iDiscDiameter = g_strctParadigm.m_strctCurrentTrial.m_iDiscDiameter;
        strctTrialBackup.m_bPerfectCircles = g_strctParadigm.m_strctHandMappingParams.m_bPerfectCircles;
        ...
        strctTrialBackup.m_fStimulusOFF_MS = g_strctParadigm.m_strctCurrentTrial.m_fStimulusOFF_MS;
        strctTrialBackup.m_iMoveDistance = g_strctParadigm.m_strctCurrentTrial.m_iMoveDistance;
        strctTrialBackup.m_iMoveSpeed = g_strctParadigm.m_strctCurrentTrial.m_iMoveSpeed;
        strctTrialBackup.m_fRotationAngle = g_strctParadigm.m_strctCurrentTrial.m_fRotationAngle;
        strctTrialBackup.m_bRandomStimulusPosition = g_strctParadigm.m_strctHandMappingParameters.m_bRandomStimulusPosition;
        strctTrialBackup.m_aiStimCenter = g_strctParadigm.m_strctCurrentTrial.m_aiStimCenter;
    elseif strcmpi(strctTrialBackup.m_strTrialType,'gabor')
        strctTrialBackup.m_iMoveDistance = g_strctParadigm.m_strctCurrentTrial.m_iMoveDistance;
        strctTrialBackup.m_iFixationColor = g_strctParadigm.m_strctCurrentTrial.m_iFixationColor;
        strctTrialBackup.m_iLength = g_strctParadigm.m_strctCurrentTrial.m_iLength ;
        strctTrialBackup.m_iWidth = g_strctParadigm.m_strctCurrentTrial.m_iWidth ;
        strctTrialBackup.numFrames = g_strctParadigm.m_strctCurrentTrial.numFrames;
        strctTrialBackup.m_fRotationAngle = g_strctParadigm.m_strctCurrentTrial.m_fRotationAngle ;
        strctTrialBackup.m_bNonsymmetric = g_strctParadigm.m_strctCurrentTrial.m_bNonsymmetric;
        strctTrialBackup.m_iContrast = g_strctParadigm.m_strctCurrentTrial.m_iContrast;
        strctTrialBackup.m_iGaborFreq = g_strctParadigm.m_strctCurrentTrial.m_iGaborFreq;
        strctTrialBackup.m_iSigma = g_strctParadigm.m_strctCurrentTrial.m_iSigma;
        strctTrialBackup.m_fGaborPhase = g_strctParadigm.m_strctCurrentTrial.m_fGaborPhase;
        strctTrialBackup.AspectRatio = g_strctParadigm.m_strctCurrentTrial.AspectRatio;
        strctTrialBackup.m_afDestRectangle = g_strctParadigm.m_strctCurrentTrial.m_afDestRectangle;
    ...
    %}
    %disp(sprintf('stat server dump timer = %s', num2str(GetSecs() - g_strctParadigm.m_fLastStatServerMemoryDumpTS)));
    ...
    if g_strctParadigm.m_iLocalExperimentIter > g_strctParadigm.m_iMaxTrialsToKeepInBuffer ||...
    ...
    elseif g_strctRealTimeStatServer.m_bTrialInfoWait && ~isempty(g_strctParadigm.m_strHandMappingCommandToStatServer)
    ...
    elseif g_strctRealTimeStatServer.m_bTrialInfoWait
