...
if g_strctParadigm.m_bPlanTrialsInAdvance
    ...
    switch  g_strctParadigm.m_strTrialType
        ...
        case 'Filter'
            [strctTrial.trial] = fnPrepareFilterTrial(g_strctPTB, tmpstrctTrial);
...
function [strctTrial] = fnPrepareFilterTrial(g_strctPTB, strctTrial)
global g_strctParadigm
strctTrial.m_bUseStrobes = 0;

strctTrial.m_iFixationColor = [255 255 255];

strctTrial.m_iLength         = squeeze(g_strctParadigm.FilterLength.Buffer(1,:,g_strctParadigm.FilterLength.BufferIdx));
strctTrial.m_iWidth          = squeeze(g_strctParadigm.FilterWidth.Buffer(1,:,g_strctParadigm.FilterWidth.BufferIdx));
strctTrial.m_iNumberOfBars   = squeeze(g_strctParadigm.FilterNumberOfFilters.Buffer(1,:,g_strctParadigm.FilterNumberOfFilters.BufferIdx));
strctTrial.m_fStimulusON_MS  = g_strctParadigm.FilterStimulusOnTime.Buffer(1,:,g_strctParadigm.FilterStimulusOnTime.BufferIdx);
strctTrial.m_fStimulusOFF_MS = g_strctParadigm.FilterStimulusOffTime.Buffer(1,:,g_strctParadigm.FilterStimulusOffTime.BufferIdx);

% which filter, quadrant, and clut do I want to use
strctTrial.m_FilterStimulusPresetColor   = g_strctParadigm.FilterStimulusPresetColor.Buffer(1,:,g_strctParadigm.FilterStimulusPresetColor.BufferIdx);
strctTrial.m_FilterBackgroundPresetColor = g_strctParadigm.FilterBackgroundPresetColor.Buffer(1,:,g_strctParadigm.FilterBackgroundPresetColor.BufferIdx);
strctTrial.m_FilterCLutPresetColor       = g_strctParadigm.FilterCLutPresetColor.Buffer(1,:,g_strctParadigm.FilterCLutPresetColor.BufferIdx);

strctTrial.numFrames         = round(strctTrial.m_fStimulusON_MS / (g_strctPTB.g_strctStimulusServer.m_RefreshRateMS));
%strctTrial.m_iMoveDistance = squeeze(g_strctParadigm.FilterMoveDistance.Buffer(1,:,g_strctParadigm.FilterMoveDistance.BufferIdx));
strctTrial.m_iMoveSpeed      = squeeze(g_strctParadigm.FilterMoveSpeed.Buffer(1,:,g_strctParadigm.FilterMoveSpeed.BufferIdx));


strctTrial.m_bBlur           = squeeze(g_strctParadigm.FilterBlur.Buffer(1,:,g_strctParadigm.FilterBlur.BufferIdx));
strctTrial.numberBlurSteps   = round(squeeze(g_strctParadigm.FilterBlurSteps.Buffer(1,:,g_strctParadigm.FilterBlurSteps.BufferIdx)));
...
strctTrial.m_bUseBlackBackground              = g_strctParadigm.m_bUseBlackBackground;  
...
elseif g_strctParadigm.m_bCycleStimulusOrientation
    ...
    strctTrial.m_fRotationAngle  = squeeze(g_strctParadigm.FilterOrientation.Buffer(1,:,g_strctParadigm.FilterOrientation.BufferIdx));
end
...
if any(TestObjects)
    ...
    for iTestObjects = 1:TestObjects
        ...
        switch strctTrial.m_acCurrentlyVariableFields{iTestObjects,1}
            case {'PlainBarLength', 'MovingBarLength', 'FilterLength'}
                strctTrial.m_iLength = round(newVal);
                
            case {'PlainBarWidth', 'MovingBarWidth', 'FilterWidth'}
                strctTrial.m_iWidth = round(newVal);
                
            case {'PlainBarMoveSpeed', 'MovingBarMoveSpeed', 'FilterSpeed'}
