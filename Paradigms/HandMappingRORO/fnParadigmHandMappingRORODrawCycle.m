...
if ~isempty(acInputFromKofiko)
    ...
    switch strCommand
        ...
        case 'LoadImageSetHandMapping'
            ...
            Clut = zeros(256,3);
            normalColors = (linspace(0,1,256));
            normalColors = round(normalColors*65535);
           ...
           % g_strctDraw.m_strctTabMask = fnInitializeTexturesMask;
         
        % Build the structure dedicated to transparancy experiment:
        % 0) linear gamma table so that the later used T-lock is ok
        % 1) load the matrix file defining the quadrants the filters and
        % the ellipse positions (built with an other script)
        % 2) fill the g_strctdraw structure
        case 'LoadTextureSetFilter'
            %feval(acInputFromKofiko{1}, acInputFromKofiko{2});
            %g_strctDraw.m_ahHandles = fnLoadImageSet({'LoadImageSet'}, acInputFromKofiko{2}, acInputFromKofiko{3});
            fnStimulusServerClearTextureMemory();
            ...
            [g_strctDraw.m_strctStim, ...
             g_strctDraw.m_strctDisp, ...   
             g_strctDraw.m_a2iTextureSize,...
             g_strctDraw.m_abIsMovie, ...
             g_strctDraw.m_aiApproxNumFrames, ...
             ~, ...
             g_strctDraw.m_acImages] = ...
             fnInitializeTexturesFilter(acInputFromKofiko{2});
			 
             g_strctDraw.WhichStim = 1;
             g_strctDraw.WhichcLut = 1;
           % g_strctDraw.m_strctTabMask = fnInitializeTexturesMask;           
...
% ---------------------------------------------------------------------------------------------------------------------
% ---------------------------------------------------------------------------------------------------------------------
function fnDisplayFilter()
global g_strctPTB g_strctDraw g_strctServerCycle
% 	dbstop if warning
% 	warning('stop')
%sprintf('dkl ID = %i',g_strctDraw.m_strctTrial.m_iCurrentlySelectedDKLCoordinateID)

fprintf('Filter n*%d, Background n*%d \n',g_strctDraw.m_strctTrial.m_FilterStimulusPresetColor, g_strctDraw.m_strctTrial.m_FilterBackgroundPresetColor);
fprintf('cLut n*%d \n\n',g_strctDraw.m_strctTrial.m_FilterCLutPresetColor);
...
% Step 1: Draw the mask texture, but *only* into the alpha channel. Don't touch
    ...
    for iNumOfBars = 1:g_strctDraw.m_strctTrial.m_iNumberOfBars            
        for iBlurStep = 1:g_strctDraw.m_strctTrial.numberBlurSteps                
            if any(g_strctDraw.m_strctTrial.coordinatesX(1:4,g_strctDraw.m_iFrameCounter,iNumOfBars,iBlurStep))
               Screen('BlendFunction', g_strctPTB.m_hWindow, GL_ONE, GL_ZERO, [0 0 0 1]); % => use [1 1 1 1] without drawing the images to visualize the mask  

               Screen('FillPoly',g_strctPTB.m_hWindow, [0 0 0 0],...%,g_strctDraw.m_strctTrial.m_aiBlurStepHolder(2,iBlurStep),g_strctDraw.m_strctTrial.m_aiBlurStepHolder(3,iBlurStep)] ,...
               ...
               % can put any values here in the first three RGB entries since I code for alpha
               % value 4 codes for transparancy then of the next called texture
               % with 0 full transparancy. So here I fill transparent values where
               % I want to, opaque elsewhere.	                   
            end
        end
    end
else
...
% Step 2: Draw peripheral image (aka the background). It is only/increasingly drawn where
% the alpha-value in the backbuffer is 1.0 or close (see above fillOval call), leaving
...
% affected:	
Screen('BlendFunction', g_strctPTB.m_hWindow, GL_DST_ALPHA, GL_ZERO, [1 1 1 0]);

 if g_strctDraw.m_strctTrial.m_bUseBlackBackground
    Screen('FillRect', g_strctPTB.m_hWindow, [255 255 255], [g_strctPTB.m_aiRect(1)-1 g_strctPTB.m_aiRect(2)-1 g_strctPTB.m_aiRect(3)+1 g_strctPTB.m_aiRect(4)+1]); 
 else    
    Screen('DrawTexture', g_strctPTB.m_hWindow,  g_strctDraw.m_strctStim(g_strctDraw.m_strctTrial.m_FilterBackgroundPresetColor).Back.Disp,...
         [], []);       
 end
...
Screen('BlendFunction', g_strctPTB.m_hWindow, GL_ONE_MINUS_DST_ALPHA, GL_ONE, [1 1 1 0]);  
Screen('DrawTexture', g_strctPTB.m_hWindow,   g_strctDraw.m_strctStim(g_strctDraw.m_strctTrial.m_FilterBackgroundPresetColor).Filter(g_strctDraw.m_strctTrial.m_FilterStimulusPresetColor).Disp,...
        [], []); 


% Step 4: Go back to normal mode so that we can load the lut and other stuffs
...
ClutTextureIndex = Screen( 'MakeTexture', ...
                           g_strctPTB.m_hWindow, ...
                           uint8(g_strctDraw.m_strctStim(g_strctDraw.m_strctTrial.m_FilterBackgroundPresetColor).Filter(g_strctDraw.m_strctTrial.m_FilterStimulusPresetColor).cLUTrand(g_strctDraw.m_strctTrial.m_FilterCLutPresetColor).cLUT) );
% Optional or deprecated ways to use clut
                           %    cLUT = BitsPlusEncodeClutRow([repmat(round(linspace(0,1,256)'.^(1/2.2) .* (2^16 -1)),1,3)]);
%    ClutTextureIndex = Screen( 'MakeTexture', g_strctPTB.m_hWindow, uint8(cLUT));
%    ClutTextureIndex = Screen( 'MakeTexture', g_strctPTB.m_hWindow, uint8(g_strctDraw.m_strctDisp.aDispLut{g_strctDraw.WhichStim, g_strctDraw.WhichcLut}));
 Screen( 'DrawTexture', g_strctPTB.m_hWindow, ClutTextureIndex, [], [46, 0, 570, 1] );
 Screen('Close',ClutTextureIndex);



%g_strctServerCycle.m_fLastFlipTime = fnFlipWrapperCLUT(g_strctPTB.m_hWindow); % Time onset of trial, blocking
...
if  g_strctDraw.WhichStim < size(g_strctDraw.m_strctDisp.aDispTable, 1) && g_strctDraw.WhichcLut == 3;
    g_strctDraw.WhichStim = g_strctDraw.WhichStim + 1;
    g_strctDraw.WhichcLut = 1;    
elseif g_strctDraw.WhichcLut < 3
    g_strctDraw.WhichcLut = g_strctDraw.WhichcLut + 1;
else
    g_strctDraw.WhichStim = 1;
    g_strctDraw.WhichcLut = 1;
end
...
% ---------------------------------------------------------------------------------------------------------------------
% ---------------------------------------------------------------------------------------------------------------------
function fnKeepDisplayingFilter()
global g_strctPTB g_strctDraw g_strctServerCycle
...
% we shouldn't be able to get larger than this number, but just in case
 
    Screen('FillArc',g_strctPTB.m_hWindow,[255 255 255], aiFixationRect, 0, 360);
    
    if g_strctDraw.m_strctTrial.m_bShowPhotodiodeRect
        Screen('FillRect',g_strctPTB.m_hWindow,[0 1 0], ...
            [g_strctPTB.m_aiRect(3)-g_strctDraw.m_strctTrial.m_iPhotoDiodeWindowPix ...
            g_strctPTB.m_aiRect(4)-g_strctDraw.m_strctTrial.m_iPhotoDiodeWindowPix ...
            g_strctPTB.m_aiRect(3) g_strctPTB.m_aiRect(4)]);
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
	% Display the stimulus, background and transparent filter
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 	
	...
	% Step 2: Draw peripheral image (aka the background). It is only/increasingly drawn where
	% the alpha-value in the backbuffer is 1.0 or close (see above fillPoly call), leaving
	% the foveated area (low or zero alpha values) alone:
	% This is done by weighting each color value of each pixel
	% with the corresponding alpha-value in the backbuffer
	% (GL_DST_ALPHA). Disable alpha channel writes via [1 1 1 0], so
...
%	Screen('FillRect', g_strctPTB.m_hWindow, [255 255 255], [g_strctPTB.m_aiRect(1) g_strctPTB.m_aiRect(2) g_strctPTB.m_aiRect(3)+1 g_strctPTB.m_aiRect(4)+1]);
...
%    g_strctServerCycle.m_fLastFlipTime = Screen('Flip', g_strctPTB.m_hWindow); % Time onset of trial, blocking
    
    % Update information
    g_strctDraw.m_a2fFrameFlipTS(1,g_strctDraw.m_iFrameCounter) = ...
    g_strctDraw.m_a2fFrameFlipTS(1,1) + (g_strctDraw.m_iFrameCounter*(1000/g_strctPTB.m_iRefreshRate));   
	% How long has it been since we started this presentation, estimated
    g_strctDraw.m_a2fFrameFlipTS(2,g_strctDraw.m_iFrameCounter) = g_strctServerCycle.m_fLastFlipTime; 
	% What is the actual time, so we can compare with the estimated if necessary
...
for iNumOfDots = 1:g_strctDraw.m_strctTrial.NumberOfDots
	...
	Screen('FillOval', g_strctPTB.m_hWindow, g_strctDraw.m_strctTrial.m_aiStimColor,  squeeze([g_strctDraw.m_strctTrial.coordinatesX(1, g_strctDraw.m_iFrameCounter, :),... left
        ...
        function fnDisplayImagebak()
            ...
            function fnKeepDisplayingImagebak()
                global g_strctPTB g_strctDraw g_strctServerCycle
...
%             Screen('LoadNormalizedGammaTable',  g_strctPTB.m_hWindow, yellowCLUT, 2); 
...
%               % draw tlock before any alpha blending  
%              Screen('BlendFunction', g_strctPTB.m_hWindow, GL_ONE, GL_ZERO, [1 1 1 0]); % => use [1 1 1 1] without drawing the images to visualize the mask                               
%                 cLUT = BitsPlusEncodeClutRow([repmat((linspace(0,1,256)').^(5),1,2) zeros(256,1)]);
%                 ClutTextureIndex = Screen( 'MakeTexture', g_strctPTB.m_hWindow, cLUT );
%                 Screen( 'DrawTexture', g_strctPTB.m_hWindow, ClutTextureIndex, [], [0, 0, 524, 1] );
%                 Screen('Close',ClutTextureIndex);                
           ...
           % Draw the mask
            ...
            % for drawing into it:
            Screen('BlendFunction', g_strctPTB.m_hWindow, GL_ONE, GL_ZERO, [0 0 0 1]); % => use [1 1 1 1] without drawing the images to visualize the mask
            %Screen('FillOval', g_strctPTB.m_hWindow, [0 0 0 0], g_strctParadigm.m_strctMRIStim.m_strtTabMask(1).Rect); % can put any values here in the first three RGB entries since I code for alpha
            Screen('FillOval', g_strctPTB.m_hWindow, [0 0 0 0], g_strctDraw.m_strctTabMask(g_strctDraw.m_iFrameCounter).Rect); % can put any values here in the first three RGB entries since I code for alpha
            % value 4 codes for transparancy then of the next called texture
            % with 0 full transparancy. So here I fill transparent values where
            % I want to, opaque elsewhere.
 ...
 %           Screen('FillRect',g_strctPTB.m_hWindow, g_strctParadigm.m_strctCurrentTrial.m_afLocalBackgroundColor, StimServerScreenRect);            
            Screen('DrawTexture', g_strctPTB.m_hWindow,  g_strctDraw.m_ahHandles(g_strctDraw.m_strctTrial.m_iImageIndex),...
                    [], g_strctDraw.m_strctTrial.m_aiStimulusArea, g_strctDraw.m_strctTrial.m_fRotationAngle + 180);
            ...
            Screen('BlendFunction', g_strctPTB.m_hWindow, GL_ONE, GL_ZERO); % => use [1 1 1 1] without drawing the images to visualize the mask               
                cLUT = BitsPlusEncodeClutRow([repmat(round(linspace(0,1,256)'.^(1/2.2) .* (2^16 -1)),1,3)]);
                ClutTextureIndex = Screen( 'MakeTexture', g_strctPTB.m_hWindow, uint8(cLUT));
                Screen( 'DrawTexture', g_strctPTB.m_hWindow, ClutTextureIndex, [], [62, 62, 586, 63] );
               Screen('Close',ClutTextureIndex);
...
%                 normalColors = round(normalColors*65535)';
%              
%                 BitsPlusSetClut(g_strctPTB.m_hWindow, [normalColors normalColors zeros(256,1)]);
         ...
         %   yellowCLUT = [RedCLUTValues; GreenCLUTValues; BlueCLUTValues]';
          %  Screen('LoadNormalizedGammaTable',  g_strctPTB.m_hWindow, yellowCLUT, 2); 
                ...
                if g_strctDraw.m_strctTrial.m_iImageIndex > numel(g_strctDraw.m_ahHandles)
%               % draw tlock before any alpha blending  
%                 Screen(g_strctPTB.m_hWindow,'BlendFunction',GL_ONE, GL_ZERO);
%                 cLUT = BitsPlusEncodeClutRow([repmat((linspace(0,1,256)').^(5),1,2) zeros(256,1)]);
%                 ClutTextureIndex = Screen( 'MakeTexture', g_strctPTB.m_hWindow, cLUT );
%                 Screen( 'DrawTexture', g_strctPTB.m_hWindow, ClutTextureIndex, [0, 0, 524, 1], [0, 0, 524, 1], 0, 0);
%                 Screen('Close',ClutTextureIndex);                        
%                     
