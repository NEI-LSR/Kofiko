...
g_strctParadigm.m_strctMRIStim.m_aiMaxImageSize = zeros(1,2);
g_strctParadigm.m_strctMRIStim.m_aiMinImageSize = zeros(1,2);
...
if g_strctPTB.m_bRunningOnStimulusServer
	...
	g_strctParadigm.m_strctMRIStim.m_aiStimulusDisplayCount = [];
end
fnHandMappingEraseOpenTextures();
g_strctParadigm.m_strctMRIStim.acFileNames = {};

for iStrSelectedImageList = 1:numel(strSelectedImageList)
    acFileNames = {};
    imageSetDir = dir([ImageDirectoryPath,'\',strSelectedImageList{iStrSelectedImageList}]);
    filetype = {};
    ...
    for iImages = 1:size(acFileNamesTemp,1)
        acFileNames{iImages} =  [ImageDirectoryPath,'\',strSelectedImageList{iStrSelectedImageList},'\',acFileNamesTemp(iImages).name];
	%g_strctParadigm.m_strctMRIStim.acFileNames{iImages} = [ImageDirectoryPath,'\',strSelectedImageList{iStrSelectedImageList},'\',acFileNamesTemp(iImages).name];
    end
g_strctParadigm.m_strctMRIStim.acFileNames = horzcat(g_strctParadigm.m_strctMRIStim.acFileNames, acFileNames);
...
g_strctParadigm.m_strctMRIStim.m_aiMaxImageSize(1) = max(a2iTextureSize(1,:));
g_strctParadigm.m_strctMRIStim.m_aiMaxImageSize(2) = max(a2iTextureSize(2,:));
g_strctParadigm.m_strctMRIStim.m_aiMinImageSize(1) = min(a2iTextureSize(1,:));
g_strctParadigm.m_strctMRIStim.m_aiMinImageSize(2) = min(a2iTextureSize(2,:));
return
