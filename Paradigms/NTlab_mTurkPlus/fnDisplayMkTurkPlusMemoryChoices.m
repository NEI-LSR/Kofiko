function [fMemoryChoicesOnsetTS] = fnDisplayMkTurkPlusMemoryChoices(hPTBWindow, strctCurrentTrial,bFlip,bClear)
global  g_strctPTB
% Clear screen
...
%}    
    ...
    Screen('DrawTexture', g_strctPTB.m_hWindow, g_strctDraw.achromcloud_disp(strctCurrentTrial.m_strctMemoryPeriod.stimseq(strctCurrentTrial.m_iMemoryFrameCounter)), [] , strctCurrentTrial.m_strctTrialParams.m_aiMkTurkCueETStimWin',0,0);
	...
	if g_strctDraw.m_strctCurrentTrial.m_iMemoryFrameCounter > length(strctCurrentTrial.m_strctMemoryPeriod.stimseq)
	...
	global g_strctParadigm
	thisFrame = rem(strctCurrentTrial.m_iLocalMemoryFrameCounter, strctCurrentTrial.m_strctMemoryPeriod.numFrames);
		thisFrame(thisFrame == 0) = 1;
%{
	for iChoices = 1:numel(strctCurrentTrial.m_strctChoiceVars.m_iChoiceTextureIDs)
       thisChoicesFrameID(iChoices) = ...
...
%    Screen('DrawTexture', g_strctPTB.m_hWindow, g_strctParadigm.achromcloud_local(g_strctParadigm.m_strctCurrentTrial.m_strctChoicePeriod.stimseq(g_strctParadigm.m_strctCurrentTrial.m_iLocalChoiceFrameCounter)), [] , g_strctPTB.m_fScale * strctCurrentTrial.m_strctTrialParams.m_aiMkTurkCueETStimWin');
    Screen('DrawTexture', g_strctPTB.m_hWindow, g_strctParadigm.achromcloud_local(g_strctParadigm.m_strctCurrentTrial.m_strctMemoryPeriod.stimseq(g_strctParadigm.m_strctCurrentTrial.m_iLocalMemoryFrameCounter)), [] , g_strctPTB.m_fScale * g_strctParadigm.m_aiETStimulusRect',0,0);
...
% 		[] ,g_strctPTB.m_fScale * strctCurrentTrial.m_aiChoiceScreenCoordinates');
    ...
    %fnDrawFixationSpot(g_strctPTB.m_hWindow, strctCurrentTrial.m_strctChoicePeriod, false, 1);
    fChoicesOnsetTS = nan;
    g_strctParadigm.m_strctCurrentTrial.m_iLocalMemoryFrameCounter = thisFrame + 1;
