function [strctOutput] = fnNTlab_mTurkPlusCycle(strctInputs, varargin)
%
...
% Move ET stimulus around
fnVariableUpdateCheck();
if isfield(g_strctParadigm, 'ETStimulusArea') && isfield(g_strctParadigm, 'ETStimulusPosition')

cur_secondarystimPos=fnTsGetVar('g_strctParadigm','ETStimulusPosition');
cur_secondarystimarea=fnTsGetVar('g_strctParadigm','ETStimulusArea')/2;

% g_strctParadigm.m_aiETStimulusRect(1) = round(cur_secondarystimPos(1)- cur_secondarystimarea);
% g_strctParadigm.m_aiETStimulusRect(2) = round(cur_secondarystimPos(2)- cur_secondarystimarea);
% g_strctParadigm.m_aiETStimulusRect(3) = round(cur_secondarystimPos(1)+ cur_secondarystimarea);
% g_strctParadigm.m_aiETStimulusRect(4) = round(cur_secondarystimPos(2)+ cur_secondarystimarea);

g_strctParadigm.m_aiETStimulusRect(1) = g_strctParadigm.m_aiCenterOfETStimulus(1)-(squeeze(g_strctParadigm.ETStimulusArea.Buffer(1,:,g_strctParadigm.ETStimulusArea.BufferIdx)/2));
g_strctParadigm.m_aiETStimulusRect(2) = g_strctParadigm.m_aiCenterOfETStimulus(2)-(squeeze(g_strctParadigm.ETStimulusArea.Buffer(1,:,g_strctParadigm.ETStimulusArea.BufferIdx)/2));
g_strctParadigm.m_aiETStimulusRect(3) = g_strctParadigm.m_aiCenterOfETStimulus(1)+(squeeze(g_strctParadigm.ETStimulusArea.Buffer(1,:,g_strctParadigm.ETStimulusArea.BufferIdx)/2));
g_strctParadigm.m_aiETStimulusRect(4) = g_strctParadigm.m_aiCenterOfETStimulus(2)+(squeeze(g_strctParadigm.ETStimulusArea.Buffer(1,:,g_strctParadigm.ETStimulusArea.BufferIdx)/2));

    [cuesize_x, cuesize_y] = deal(fnTsGetVar('g_strctParadigm','CueWidth'));
    g_strctParadigm.m_aiMkTurkCueRect=[g_strctParadigm.m_aiCenterofCueWin(1)-cuesize_x;...
        g_strctParadigm.m_aiCenterofCueWin(2)-cuesize_y;...
        g_strctParadigm.m_aiCenterofCueWin(1)+cuesize_x;...
        g_strctParadigm.m_aiCenterofCueWin(2)+cuesize_y];
end
...
switch g_strctParadigm.m_iMachineState
    ...
    case 2
        ...
        if (~isempty(g_strctParadigm.m_strctCurrentTrial) && isfield(g_strctParadigm.m_strctCurrentTrial,'m_strctTrialOutcome') && ~g_strctParadigm.m_bDynamicStimuli && ...
            ...
            %tic
            [strctTemp,strWhatHappened] =  fnNTlab_mTurkPlusPrepareTrial();
        ...
        if ~fnParadigmToKofikoComm('IsTouchMode')
            ...
            fnNTlab_mTurkPlusDrawCycle({'PrepareTrial',g_strctParadigm.m_strctCurrentTrial});
        end
    ...
    case 4
        ...
        if ~isempty(g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod)
            ...
            if ~fnParadigmToKofikoComm('IsTouchMode')
                fnParadigmToStimulusServer('ShowFixationCircle'); %Felix edit
%                fnParadigmToStimulusServer('ShowFixationSpot');
                ...
                %fFlipTimeLocal = fnNTlab_mTurkPlusDrawCycle({'ShowFixationSpot'});
                fFlipTimeLocal = fnNTlab_mTurkPlusDrawCycle({'ShowFixationCircle'});
...
%         warning('stop')
            ...
            if  strctInputs.m_abMouseButtons(1)
                ...
                if fDistToFixationSpot < g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod.m_fFixationRegionPix
                    ...
                    if g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod.m_bAbortTrialUponTouchOutsideFixation
                        ...
                        fnNTlab_mTurkPlusDrawCycle({'ClearScreen'});
    ...
    case 10
        ...
        if isempty(g_strctParadigm.m_strctCurrentTrial.m_strctCuePeriod) && isempty(g_strctParadigm.m_strctCurrentTrial.m_astrctChoicesMedia)
            ...
            if fnParadigmToKofikoComm('IsTouchMode')
                fnNTlab_mTurkPlusDrawCycle({'StartTrial'});
    ...
    case 12
        ...
        [strWhatHappened, fWhenItHappened] = fnNTlab_mTurkPlusDrawCycle({''});
    ...
    case 14
        ...
        fnNTlab_mTurkPlusDrawCycle({'AbortTrial'}); % Clear screen & handles
        feval(g_strctParadigm.m_strCallbacks,'TrialOutcome',g_strctParadigm.m_strctCurrentTrial);
        fnTsSetVarParadigm('acTrials',g_strctParadigm.m_strctCurrentTrial);
    ...
    case 15
        ...
        if bGiveJuice
            ...
            % Correct trial!
            fnNTlab_mTurkPlusDrawCycle({'AbortTrial',...
            ...
            if g_strctParadigm.m_strctCurrentTrial.m_strctChoicePeriod.m_bMultipleAttemptsUntilJuice
                ...
                if g_strctParadigm.m_strctCurrentTrial.m_strctPostTrial.m_bExtinguishNonSelectedChoicesAfterChoice
                    fnNTlab_mTurkPlusDrawCycle({'AbortTrial',g_strctParadigm.m_iSelectedChoice, g_strctParadigm.m_strctCurrentTrial.m_strctPostTrial.m_fRetainSelectedChoicePeriodMS});
                else
                    fnNTlab_mTurkPlusDrawCycle({'AbortTrial'});
                end
                fnTsSetVarParadigm('acTrials',g_strctParadigm.m_strctCurrentTrial);
    ...
    case 20
        ...
        if bAbortTrial
            ...
            if ~isempty(strctInputs.m_acInputFromStimulusServer)
                ...
                elseif strcmpi(strctInputs.m_acInputFromStimulusServer{1},'MemoryOnset')
                    % Felix TODO: keep draw function involved here? Nah,
                    % draw is maintained in DrawCycle
                elseif strcmpi(strctInputs.m_acInputFromStimulusServer{1},'MemoryChoiceOnset')
                    g_strctParadigm.MemoryChoiceBool = 1;
                end
            end
        end
        ...
        if euclidianDistance < g_strctParadigm.m_strctCurrentTrial.m_iChoiceRingSize
            if strcmpi(g_strctParadigm.m_strctChoiceVars.m_strChoiceDisplayType, 'ring')
                ...
                if isempty(insideRhoSat)
                    ...
                    g_strctParadigm.m_fInsideChoiceTS = GetSecs();
                    g_strctParadigm.m_strctCurrentTrial.m_iSelectedChoice = g_strctParadigm.m_strctCurrentTrial.m_iCurrentlySelectedChoice; %felix added
...
% First part of this is largely defunt
% Second part is critically important to the hand mapping paradigm. If trials are not
% sent to the stat server and cleared periodically, the control computer's memory will fill
% and the program will stop working eventually. This function serves the dual purpose of 
% sending periodic updates to the stat server for saving to disk, and allows the stat server to
% parse trials in realtime to provide user feedback
...
%    if ~isempty(g_strctParadigm.m_strctAllPlannedTrials(iTrialsDisplayed).trial.m_fImageFlipON_TS_StimulusServer) && ~isempty(g_strctParadigm.m_strctAllPlannedTrials(iTrialsDisplayed).trial.m_fImageFlipON_TS_Kofiko)
...
%%%%
%%%%
% Updates to the stat server are designed to occur immediately following a screen flip on the control computer
% This is to prevent situations in which the update will send, occupying time on the control computer, and then 
% the screen flipping, which will occupy more time, during which a trial prep might be skipped. 
% since trial prep occurs before this backup is called, the reverse should not occur
...
% Parses inputs from the GUI and passes them to the stat server as part of the trial update
% Add new if statement here if new conditions are added to the stat server or the GUI
...
if g_strctParadigm.m_strctStatServerComm.m_bToggleWidthPlot
    g_strctParadigm.m_strHandMappingCommandToStatServer(iCommandsInBuffer).cmd = {'ToggleVariableWidthPlot'};
    iCommandsInBuffer = iCommandsInBuffer+1;
    g_strctParadigm.m_strctStatServerComm.m_bToggleWidthPlot = 0;
    
end

if g_strctParadigm.m_strctStatServerComm.m_bToggleLengthPlot
    g_strctParadigm.m_strHandMappingCommandToStatServer(iCommandsInBuffer).cmd = {'ToggleVariableLengthPlot'};
    iCommandsInBuffer = iCommandsInBuffer+1;
    g_strctParadigm.m_strctStatServerComm.m_bToggleLengthPlot = 0;
    
end

if g_strctParadigm.m_strctStatServerComm.m_bToggleSpeedPlot
    g_strctParadigm.m_strHandMappingCommandToStatServer(iCommandsInBuffer).cmd = {'ToggleVariableSpeedPlot'};
    iCommandsInBuffer = iCommandsInBuffer+1;
...
if g_strctParadigm.m_strctStatServerComm.m_bToggleImagePlot
    g_strctParadigm.m_strHandMappingCommandToStatServer(iCommandsInBuffer).cmd = {'ToggleImagePlot'};
    iCommandsInBuffer = iCommandsInBuffer+1;
    g_strctParadigm.m_strctStatServerComm.m_bToggleImagePlot = 0;
    
end

if g_strctParadigm.m_strctStatServerComm.m_bClearImageBuffer
    g_strctParadigm.m_strHandMappingCommandToStatServer(iCommandsInBuffer).cmd = {'FlushImageSpikeBuffer'};
    iCommandsInBuffer = iCommandsInBuffer+1;
    g_strctParadigm.m_strctStatServerComm.m_bClearImageBuffer = 0;
    
end

if g_strctParadigm.m_strctStatServerComm.m_bPrintStatFigure
    g_strctParadigm.m_strHandMappingCommandToStatServer(iCommandsInBuffer).cmd = {'PrintFigure'};
    iCommandsInBuffer = iCommandsInBuffer+1;
