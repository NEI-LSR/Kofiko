...
experimentStimulusVars.CueCoordinates = [xpos, ypos];
experimentStimulusVars.CueSize = theta;
experimentStimulusVars.BarDimensions = [length, width];
experimentStimulusVars.BarBlur = [blur, blurSteps];
% Stimulation stuff
...
function [RGB] =  getRGBvalsG(values, conditions, neutralGray, neuronPeakColor)
% Gets RGB values from color .mat file and correlates them with the correct
% stimuli. This version is for Gabor generation, but is backward compatible
% with the regular bar generation function



% Gets RGB information from file and associates the index of the condition
% in the condition cell with that color
RGB = [];
allColors = [];

% Setup spatial frequency color selection. Start as -1 so we can check if
% it is changed later and not have to repeatedly ask the user for a value.

numConditions = numel(conditions);
for i = 1:numConditions
	if strcmp(conditions{i},'choices')
		[m, ~] = size(neuronPeakColor);
	else
		[m, ~] = size(values.(conditions{i}).RGB);
    end
	[q, ~] = size(allColors);
    for x = q+1:q+m
		% ========================================================================================================
		% Color for the choices
		if strcmp(conditions{i},'choices')
			allColors(x,1:3) = values.(neuronPeakColor{x-q,2}).RGB(neuronPeakColor{x-q,1},1:3);
			allColors(x,4) = i;
			allColors(x,5) = x-q;
		else
		% ========================================================================================================
		% Colors for regular bars
			allColors(x,1:3) = values.(conditions{i}).RGB(x-q,1:3);
			allColors(x,4) = i;
			allColors(x,5) = x-q;
        end
