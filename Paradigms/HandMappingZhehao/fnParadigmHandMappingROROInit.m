...
%% Initialization of few parameters for the current paradigm
iSmallBuffer = 500;
iLargeBuffer = 50000;
...
%% Color table initialization for the current paradigm
% Loads all needed colors that will be displayed
%{
...
% Load LUT for linar gamma
g_strctParadigm.m_strctGammaCorrectedLookupTable = load('gammaCorrectedLookUpTable');
% Load DKL matrices (see Qasim Zaidi calib.m and ldrgyv2rgb fuunction for
% details
...
% Load relative saturations of the stim to display
...
for iSaturations = 1:numel(g_strctParadigm.m_aiPresetSaturations)
    % build a circle of hues with XX steps defined with angle values
    % DEFINED IN THE DKL COLOR SPACE
	g_strctParadigm.m_strctMasterColorTable.(g_strctParadigm.m_acstrSaturationsLookup{iSaturations}).azimuthSteps = ...
									deg2rad(0:360/g_strctParadigm.m_fInitial_SaturationsAzimuthSteps:359.99); % is it to avoid a bug here???
    ...
    % save the polar and cartesian coordinates in two tables    
    ...
    % COMPUTE THE SCREEN RGB VALUES to display the desired DKL values
    uncorrectedRGB = floor(ldrgyv2rgb(ones(1,numel(x)) * g_strctParadigm.m_afInitial_SaturationsElevation(iSaturations),x,y)*65535);
    
    % Use the right values knowing that the screen is not Linear (Could
    % load this correction before with the PsychImaging option maybe)
...
% Compute for the desired luminance backgrounds
...
%% For Filter
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterNumberOfFilters', g_strctParadigm.m_fInitial_FilterNumberOfFilters, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterWidth', g_strctParadigm.m_fInitial_FilterWidth, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterLength', g_strctParadigm.m_fInitial_FilterLength, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterOrientation', g_strctParadigm.m_fInitial_FilterOrientation, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterMoveDistance', g_strctParadigm.m_fInitial_FilterMoveDistance, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterStimulusArea', g_strctParadigm.m_fInitial_FilterStimulusArea, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterStimulusRed', g_strctParadigm.m_fInitial_FilterStimulusRed, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterStimulusGreen', g_strctParadigm.m_fInitial_FilterStimulusGreen, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterStimulusBlue', g_strctParadigm.m_fInitial_FilterStimulusBlue, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterBackgroundRed', g_strctParadigm.m_fInitial_FilterBackgroundRed, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterBackgroundGreen', g_strctParadigm.m_fInitial_FilterBackgroundGreen, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterBackgroundBlue', g_strctParadigm.m_fInitial_FilterBackgroundBlue, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterBlur', g_strctParadigm.m_fInitial_FilterBlur, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterBlurSteps', g_strctParadigm.m_fInitial_FilterBlurSteps, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterMoveSpeed', g_strctParadigm.m_fInitial_FilterMoveSpeed, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterStimulusOffTime', g_strctParadigm.m_fInitial_FilterStimulusOffTime, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterStimulusOnTime', g_strctParadigm.m_fInitial_FilterStimulusOnTime, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterStimulusPresetColor', g_strctParadigm.m_fInitial_FilterStimulusPresetColor, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterBackgroundPresetColor', g_strctParadigm.m_fInitial_FilterBackgroundPresetColor, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterCLutPresetColor', g_strctParadigm.m_fInitial_FilterCLutPresetColor, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterCurrentVariableModifySpeedHz', g_strctParadigm.m_fInitial_CurrentVariableModifySpeedHz, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FilterCurrentVariableModifyRange', g_strctParadigm.m_fInitial_CurrentVariableModifyRange, iSmallBuffer);

g_strctParadigm.m_bUseFilterPresetColors = 0;
g_strctParadigm.m_bUseBlackBackground    = 1;
...
%Im2 = (FinalPic(3).Quadrant(4).Filter.RGBN).^(1/2.2);
...
% end
% 
% g_strctParadigm.m_strctMRIStim.m_strtTabMask = Tab;
...
g_strctParadigm.m_strctFilterDesignBackup = g_strctParadigm.m_strctDesign.m_strctBlocksAndOrder;
