...
load([acFileNames,filesep,'2017-4-24-13g_strctParadigm.mat']);
...
% Build as many filters as needed, sinusoidal modulation as a EE SPD.
Step = 20;
...
for ii = 0 : Step : 359
    ...
    strctList.CC.List = [strctList.CC.List temp];
     strctList.CC.Ind = [strctList.CC.Ind  ii+2];   
end
...
%strctList.CC.Ind        = strctList.CC.List;
...
for ii = 1 : length(strctList.Quadrant.Ind)
     ...
     % Do not forget the minus 1 since you load pictures and not index
     % values
     ...
     RGBtemp =  mstrctSurfaceProperties.afRGB(ColorMin:ColorMax,:);
...
%     ImText(ii).Back.Disp = Screen('MakeTexture',number, uint8(round(ImTemp)));
    ...
    for jj = 1 : length(strctList.CC.Ind)
        ...
        % complete list of 280 colors computed in SpotlightOpt_Main.m)        
         RGBtemp =  mstrctObjectProperties(strctList.CC.Ind(jj)).afRGB(ColorMin:ColorMax,:);
		 ...
		 % Build basic colors to be used all the time
		 cLUTtemp(256,:) = [0 0 0];
		 cLUTtemp(255,:) = [1 1 1];
		 cLUTtemp(254,:) = [.5 .5 .5];
		 cLUTtemp(253,:) = [1 0 0];
...
% 		 cLUTtemp(1,:) = [1 0 1];	
% 		 cLUTtemp(41,:) = [1 1 0];	
% to activate for testing/debugging
         ...
         for kk = 1 : 10
		    ...
		    cLUTtemprand(256,:) = [0 0 0];
		    cLUTtemprand(255,:) = [1 1 1];
		    cLUTtemprand(254,:) = [.5 .5 .5];
		    cLUTtemprand(253,:) = [1 0 0];
		    cLUTtemprand(252,:) = [0 1 0];
		    cLUTtemprand(251,:) = [0 0 1];	
%  		    cLUTtemprand(1,:) = [1 0 1];	
%  		    cLUTtemprand(41,:) = [1 1 0];	             
