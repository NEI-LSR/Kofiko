function fnParadigmTouchForceChoiceDraw(bParadigmPaused)
%
...
if ~isempty(acMedia) && ~isempty(g_strctParadigm.m_strctCurrentTrial)
    ...
    if g_strctParadigm.m_iMachineState <= 6
        ...
        if ~isempty(g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod) 
              ...
              g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod.m_pt2fFixationPosition-ones(1,2)*g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod.m_fFixationRegionPix,...
              g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod.m_pt2fFixationPosition+ones(1,2)*g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod.m_fFixationRegionPix];
			% Screen('FillRect',g_strctPTB.m_hWindow ,g_strctParadigm.m_strctCurrentTrial.m_afLocalBackgroundColor);
			 	Screen('FillRect', g_strctPTB.m_hWindow,g_strctParadigm.m_strctCurrentTrial.m_strctTrialParams.m_afLocalBackgroundColor,...
							[0 0 g_strctPTB.m_fScale*g_strctParadigm.m_strctCurrentTrial.m_strctTrialParams.m_aiStimServerScreen(3),...
							g_strctPTB.m_fScale*g_strctParadigm.m_strctCurrentTrial.m_strctTrialParams.m_aiStimServerScreen(4)]);
             Screen('FrameArc', g_strctPTB.m_hWindow,g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod.m_afFixationColor, aiArcBoundingBox,0,360);
  
        end
    else
        if g_strctParadigm.m_iMachineState == 7 && ~isempty(g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod)
                fnDrawFixationSpot(g_strctPTB.m_hWindow, g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod, false,g_strctPTB.m_fScale);
                aiArcBoundingBox =g_strctPTB.m_fScale * [
                    g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod.m_pt2fFixationPosition-ones(1,2)*g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod.m_fFixationRegionPix,...
                    g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod.m_pt2fFixationPosition+ones(1,2)*g_strctParadigm.m_strctCurrentTrial.m_strctPreCuePeriod.m_fFixationRegionPix];
		...
		if g_strctParadigm.m_iMachineState == 20
        %if g_strctParadigm.m_strctCurrentTrial.m_strctTrialParams.m_bDynamicTrial && any(ismember(g_strctParadigm.m_iMachineState, [10]))
			
        end
        if ~strcmp(g_strctParadigm.m_strctCurrentTrial.m_astrctChoicesMedia.m_strChoiceType,'ring') && ~isempty( g_strctParadigm.m_strctCurrentTrial.m_astrctChoicesMedia)
        ...
        elseif strcmp(g_strctParadigm.m_strctCurrentTrial.m_astrctChoicesMedia.m_strChoiceType, 'ring') && any(ismember(g_strctParadigm.m_iMachineState, [13,15,16,17]))
			%% 13, 15, 16, 17 are the machine states corresponding to the choice period
            fnDisplayChoiceRing(g_strctPTB.m_hWindow,g_strctParadigm.m_strctCurrentTrial);
...
%}
 ...
 if g_strctParadigm.m_bPolarPlot 
	...
	Screen('FrameOval',g_strctPTB.m_hWindow, g_strctPTB.m_afPolarOutlineColors, g_strctPTB.m_afDirectionPolarRect) 
	Screen('FramePoly',g_strctPTB.m_hWindow, g_strctPTB.m_afPolarColors, g_strctParadigm.m_strctStatistics.m_afDirectionPolarPlottingArray)
	...
	%{
elseif g_strctParadigm.m_bPolarPlot
...
elseif strcmpi(g_strctParadigm.m_strCueType,'bar')
	...
	if ~g_strctParadigm.m_strctCurrentTrial.m_strctTrialParams.m_strctStimulusVariables.m_bBlur
		...
		for iNumOfBars = 1:g_strctParadigm.m_strctCurrentTrial.m_strctTrialParams.m_strctStimulusVariables.m_iNumberOfBars
			for iBlurStep = 1:g_strctParadigm.m_strctCurrentTrial.m_strctTrialParams.m_strctStimulusVariables.m_inumberBlurSteps
			
				Screen('FillPoly',g_strctPTB.m_hWindow, g_strctParadigm.m_strctCurrentTrial.m_strctTrialParams.m_aiBlurStepHolder(:,iBlurStep),...
							horzcat(g_strctParadigm.m_strctCurrentTrial.m_strctTrialParams.m_strctStimulusVariables.m_aiCoordinatesX(1:4,g_strctParadigm.m_iLocalFrameCounter,iNumOfBars,iBlurStep),...
							g_strctParadigm.m_strctCurrentTrial.m_strctTrialParams.m_strctStimulusVariables.m_aiCoordinatesY(1:4,g_strctParadigm.m_iLocalFrameCounter,iNumOfBars,iBlurStep)),0);
			end
		end
        %}
	end
...
if ~g_strctParadigm.m_strctCurrentTrial.m_bBlur
	for iNumOfBars = 1:g_strctParadigm.m_strctCurrentTrial.NumberOfBars
