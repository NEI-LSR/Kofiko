...
if strctCurrentTrial.m_strctStimuliVars.m_bDirectMatchCueChoices || rand() > (strctCurrentTrial.m_strctStimuliVars.m_fProbeTrialProbability/100)
    ...
    strctCurrentTrial.m_aiActiveChoiceColorID = [strctCurrentTrial.m_strctCuePeriod.m_iSelectedColorID, ...
        Distractors(randperm(strctCurrentTrial.m_strctChoiceVars.numColors-1,strctCurrentTrial.m_strctChoiceVars.m_NTargets-1))]; % subset random hues (including match) for each trial
else
...
if strcmpi(g_strctParadigm.m_strctChoiceVars.m_strChoiceDisplayType, 'disc') || ...
    ...
    elseif strcmpi(g_strctParadigm.m_strctChoiceVars.m_strChoiceDisplayType, 'nestedannuli')
        ...
        strctCurrentTrial.m_strctChoiceVars.m_ciQuadrantsSelected = g_strctParadigm.m_ciQuadrantsSelected;
        TotalSpan = numel(strctCurrentTrial.m_strctChoiceVars.m_ciQuadrantsSelected)*90;
        StartSpan = (90*(min(cell2mat(strctCurrentTrial.m_strctChoiceVars.m_ciQuadrantsSelected))-1));
        Separation = TotalSpan/nTotalChoices;

        if g_strctParadigm.m_strctChoiceVars.m_bRotateChoiceRingOnEachTrial
            strctCurrentTrial.m_strctChoiceVars.m_bRotateChoiceRingOnEachTrial = true;
            rotAngle = floor(rand()*Separation);
            minAngle = rotAngle + StartSpan;
            maxAngle = Separation*(strctCurrentTrial.m_strctChoiceVars.m_NTargets-1) + rotAngle + StartSpan;
            ChoiceThetas = sort(mod(linspace(minAngle, maxAngle, strctCurrentTrial.m_strctChoiceVars.m_NTargets), 360));
            strctCurrentTrial.m_strctChoiceVars.m_afChoiceThetas = circshift(ChoiceThetas, floor(rand()*strctCurrentTrial.m_strctChoiceVars.m_NTargets));
        else
            strctCurrentTrial.m_strctChoiceVars.m_bRotateChoiceRingOnEachTrial = false;
            minAngle = Separation + StartSpan;
            maxAngle = Separation*(strctCurrentTrial.m_strctChoiceVars.m_NTargets-1) + StartSpan;
            strctCurrentTrial.m_strctChoiceVars.m_afChoiceThetas = sort(mod(linspace(minAngle, maxAngle, strctCurrentTrial.m_strctChoiceVars.m_NTargets), 360));
        end
