...
switch g_strctParadigm.m_iMachineState
    ...
    case 21
        ...
        iNumChoices = length(g_strctParadigm.m_strctCurrentTrial.m_astrctChoicesMedia);
		g_strctParadigm.m_bDebugModeEnabled
        if strcmp(g_strctParadigm.m_strctCurrentTrial.m_astrctChoicesMedia.m_strChoiceType,'ring')
           ...
           if g_strctParadigm.m_bDebugModeEnabled
            ...
            if euclidianDistance < g_strctParadigm.m_strctCurrentTrial.m_iChoiceRingSize/2 
                   % eye trace is inside the choice ring
                    ...
                    choiceDirectionTheta = choiceDirectionTheta - wrapTo2Pi(deg2rad(g_strctParadigm.m_strctCurrentTrial.m_strctChoicePeriod.m_fRotationAngle));
					...
					%{
                    [selectedChoiceRho,g_strctParadigm.m_iSelectedChoiceRho] = find(g_strctParadigm.m_strctChoiceVars.choiceParameters.m_aiChoiceRho(:,1) > choiceDirectionRho & ...
                                                                        g_strctParadigm.m_strctChoiceVars.choiceParameters.m_aiChoiceRho(:,2) < choiceDirectionRho );
                   
                   if isempty(selectedChoiceRho)
                       % neutral gray case
                       selectedChoiceRho = 0;
                       g_strctParadigm.m_iSelectedChoiceThetaID = 0;
                       g_strctParadigm.m_iSelectedChoice = 0;
                   else
                        g_strctParadigm.m_iSelectedChoice = selectedChoiceRho(g_strctParadigm.m_iSelectedChoiceThetaID);
                        min(abs(choiceDirectionRho - g_strctParadigm.m_strctChoiceVars.choiceParameters.m_aiChoiceRho));
                        disp(sprintf('choice selected: %i, \n DirectionThetaID: %i, \n Selected Rho: %f',...
                            g_strctParadigm.m_iSelectedChoice,g_strctParadigm.m_iSelectedChoiceThetaID, choiceDirectionRho))
                   end
                    %}
                    g_strctParadigm.m_iMachineState = 22;
                     end
                %   choiceParameters.m_afChoiceAngles = radCoords;
                %   choiceParameters.m_iRingDimension = ringDimension;
                 %  choiceParameters.m_iRingWidth
            end
            %{
             if euclidianDistance > ...
                    g_strctParadigm.m_strctChoiceVars.choiceParameters.m_iRingDimension/2 - ...
                    g_strctParadigm.m_strctChoiceVars.choiceParameters.m_iRingWidth && ...
                    euclidianDistance < g_strctParadigm.m_strctChoiceVars.choiceParameters.m_iRingDimension/2
                    [choiceDirectionTheta, choiceDirectionRho] = cart2pol(strctInputs.m_pt2iEyePosScreen(1)-g_strctParadigm.m_strctCurrentTrial.m_afChoiceRingLocation(1), ...
                                                                            strctInputs.m_pt2iEyePosScreen(2)- g_strctParadigm.m_strctCurrentTrial.m_afChoiceRingLocation(2));
                    choiceDirectionTheta = wrapTo2Pi(choiceDirectionTheta);
                   
                    bInsideChoice = true;
                    g_strctParadigm.m_fInsideChoiceTS = GetSecs();
                    
                    [~,g_strctParadigm.m_iSelectedChoice] = min(abs(choiceDirectionTheta - g_strctParadigm.m_strctChoiceVars.choiceParameters.m_afChoiceAngles));
                     sprintf('choice selected: %i, \n DirectionTheta: %f',g_strctParadigm.m_iSelectedChoice,choiceDirectionTheta)
                    g_strctParadigm.m_iMachineState = 22;
                %   choiceParameters.m_afChoiceAngles = radCoords;
                %   choiceParameters.m_iRingDimension = ringDimension;
                 %  choiceParameters.m_iRingWidth
         ...
         if strcmp(g_strctParadigm.m_strctCurrentTrial.m_astrctChoicesMedia.m_strChoiceType,'ring')
		  if g_strctParadigm.m_bDebugModeEnabled
            ...
            if euclidianDistance <  g_strctParadigm.m_strctCurrentTrial.m_iChoiceRingSize/2
			 ...
			 bBlinkTimerElapsed = fnCheckBlinkTimer()
					if g_strctParadigm.m_fBlinkTimer == 0
						g_strctParadigm.m_fBlinkTimer = GetSecs();
						
					elseif GetSecs() - g_strctParadigm.m_fBlinkTimer>...
						fnTsGetVar('g_strctParadigm','BlinkTimeMS')/1e3)
						g_strctParadigm.m_iMachineState = 
					else
    ...
    case 23
        ...
        bGiveJuice = 0;
        [bGiveJuice, bJuiceDropMultiplier, fJuiceTimeMultiplier] = fnDetermineJuiceReward(g_strctParadigm.m_strctCurrentTrial.m_fChoiceDirectionTheta,  g_strctParadigm.m_strctCurrentTrial.m_fChoiceDirectionRho)
...
function [bGiveJuice, bJuiceDropMultiplier, fJuiceTimeMultiplier] = fnDetermineJuiceReward(choiceDirectionTheta, choiceDirectionRho)
global g_strctParadigm
dbstop if warning
warning('stop')

if g_strctParadigm.m_strctCurrentTrial.m_strctReward.m_bBinaryReward
%g_strctParadigm.m_strctCurrentTrial.m_strctReward.m_fRewardRange = .2;
