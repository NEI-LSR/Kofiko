function fnInitializeCueTrainingTextures(textureType, numTextures, numDiscs, textureSize, numEntriesPerTexture, CLUToffset, blockSize, varargin)
...
if blockSize == 1
    ...
    if g_strctPTB.m_bRunningOnStimulusServer
		...
		g_strctParadigm.m_strctStimuliVars.CueLUTs = [];
        ...
        for iDiscs = 1:numDiscs
            g_strctParadigm.m_strctStimuliVars.CueLUTs(iDiscs,:) = LastUsedClutEntry:(LastUsedClutEntry + numEntriesPerTexture)-1;
            for iTextures = 1:numTextures
            ...
            LastUsedClutEntry = max(g_strctParadigm.m_strctStimuliVars.CueLUTs(iDiscs,:))+1;
        end
        %{
        % special case, generate 1 extra disc for the null condition (gray trial)
        g_strctParadigm.m_strctStimuliVars.CueLUTs(iDiscs,:) = LastUsedClutEntry:(LastUsedClutEntry + numEntriesPerTexture)-1;
...
elseif blockSize > 1
    ...
    if g_strctPTB.m_bRunningOnStimulusServer
        ...
        g_strctParadigm.m_strctStimuliVars.CueLUTs = [];
        for iDiscs = 1:numDiscs
            g_strctParadigm.m_strctStimuliVars.CueLUTs(iDiscs,:) = ...
