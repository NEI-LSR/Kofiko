...
for iFileIter=1:length(astrctMedia)
    if  astrctMedia(iFileIter).m_bImage
		...
		[strPath, strFile, strExt] = fileparts(astrctMedia(iFileIter).m_strFileName);
		
		bMatrix = strcmpi(strExt,'.mat');
		if bMatrix
			[I, Clut] = fnReadImageWrapper(astrctMedia(iFileIter).m_strFileName);
		else
