function fnParadigmNDotCallbacks(strCallback,varargin)
%
...
switch strCallback
  ...
  case 'BlinkTimeSlider'
	...
	case 'ScreenCoverage'
		% fnDAQWrapper('StrobeWord', );
		strTemp = get(g_strctParadigm.m_strctControllers.m_hScreenCoverage,'string');
        iScreenCoverage = fnMyStr2Num(strTemp);
        if ~isempty(iScreenCoverage)
            fnUpdateSlider(g_strctParadigm.m_strctControllers.m_hScreenCoverageSlider, iScreenCoverage);
            g_strctParadigm.m_strctStimulusParams = fnTsSetVar(g_strctParadigm.m_strctStimulusParams,'ScreenCoverage',iScreenCoverage);
            fnLog('Setting new amount of screen used for task to %d percent', iScreenCoverage);
        end;
	%}	
    case 'ScreenCoverageSlider'
        % fnDAQWrapper('StrobeWord', );
        iScreenCoverage = round(get(g_strctParadigm.m_strctControllers.m_hScreenCoverageSlider,'value'));
        set(g_strctParadigm.m_strctControllers.m_hScreenCoverageEdit,'String',num2str(iScreenCoverage));
        g_strctParadigm.m_strctStimulusParams = fnTsSetVar(g_strctParadigm.m_strctStimulusParams,'ScreenCoverage',iScreenCoverage);
        if g_strctParadigm.m_iMachineState > 0 
            g_strctParadigm.m_iMachineState = 1;        
        end;
        fnLog('Setting new amount of screen used for task to %d percent', iScreenCoverage);
    case 'ScreenCoverageEdit'
        % fnDAQWrapper('StrobeWord', );
        strTemp = get(g_strctParadigm.m_strctControllers.m_hScreenCoverageEdit,'string');
        iScreenCoverage = fnMyStr2Num(strTemp);
        if ~isempty(iScreenCoverage)
            fnUpdateSlider(g_strctParadigm.m_strctControllers.m_hScreenCoverageSlider, iScreenCoverage);
            g_strctParadigm.m_strctStimulusParams = fnTsSetVar(g_strctParadigm.m_strctStimulusParams,'ScreenCoverage',iScreenCoverage);
