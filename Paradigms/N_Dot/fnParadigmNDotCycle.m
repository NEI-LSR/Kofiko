function [strctOutput] = fnParadigmNDotCycle(strctInputs, bPaused)
%
...
switch g_strctParadigm.m_iMachineState
    case 0
		...
		g_strctParadigm.CentralFix = 1; % set first fixation spot to center
    case 1
        ...
        if g_strctParadigm.m_bUpdateFixationSpot
			...
			iScreenCoverage = g_strctParadigm.m_strctStimulusParams.ScreenCoverage.Buffer(g_strctParadigm.m_strctStimulusParams.ScreenCoverage.BufferIdx);
            pt2iCenter = g_strctStimulusServer.m_aiScreenSize(3:4)/2;
			max_X = ((g_strctStimulusServer.m_aiScreenSize(3)/2) * (iScreenCoverage/100)) - iFixationSpotRad;
			max_Y = ((g_strctStimulusServer.m_aiScreenSize(4)/2) * (iScreenCoverage/100)) - iFixationSpotRad;
			
			if g_strctParadigm.CentralFix
			% fixation spot has to be in center
				pt2iNextFixationSpot = pt2iCenter; 
				g_strctParadigm.CentralFix = false; % next fixation will be in periphery
			elseif ~g_strctParadigm.CentralFix
			% fixation spot can be anywhere
				randX = (2*max_X*rand()) - max_X
				randY = (2*max_Y*rand()) - max_Y
				pt2iNextFixationSpot = [pt2iCenter(1)-randX, pt2iCenter(2)-randY]; 
				g_strctParadigm.CentralFix = true;	% next fixation wil be at center
			end				
