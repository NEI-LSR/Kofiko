...
if ~exist(strTrainFile,'file')
    strTrainFile = fullfile(strRawFolder,[strSession,'-Grass Train Out.raw']);
end
...
%iChannelID=1;
%fnAnalyzeEyeMovementsDuringMicroStimAsFunctionOfDepth(strctKofiko,strctSync,strRawFolder,strSession,iChannelID, strctAdvancersInformation,strctStatServer,strTriggerFile);
...
for iChannelIter=1:iNumSortedChannels
    ...
    for iUnitIter=1:iNumSortedUnits
        ...
        %% Add eye movement information + whether this was run during passive fixation (!)
        
        fnStandardTrainAnalysis(strSubject,strTimeDate,strctChannelInfo,astrctUnits(iUnitIter), strctAdvancerInformation, strTrainFile,strTrainFile2,strAnalogChannelFile,strOutputFolder,strOpsin,...
